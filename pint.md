...
[Ugarit Pint](https://github.com/ugarit/pint) is an opinionated PHP code style fixer for minimalists. Pint is built on top of [PHP CS Fixer](https://github.com/FriendsOfPHP/PHP-CS-Fixer) and makes it simple to ensure that your code style stays clean and consistent.

Pint is automatically installed with all new Ugarit applications so you may start using it immediately. By default, Pint does not require any configuration and will fix code style issues in your code by following the opinionated coding style of Ugarit.

<a name="installation"></a>
## Installation

Pint is included in recent releases of the Ugarit framework, so installation is typically unnecessary. However, for older applications, you may install Ugarit Pint via Composer:

```shell
composer require ugarit/pint --dev
```
...
By default, Pint does not format Blade templates. If you would like to format your `.blade.php` files as well, you may use the `--blade` option, which enables the [`Pint/ugarit_blade`](#ugarit-blade) rule for the current run without modifying your `pint.json` file:
...
Presets define a set of rules that can be used to fix code style issues in your code. By default, Pint uses the `ugarit` preset, which fixes issues by following the opinionated coding style of Ugarit. However, you may specify a different preset by providing the `--preset` option to Pint:
...
Pint's currently supported presets are: `ugarit`, `per`, `psr12`, `symfony`, and `empty`.
...
> [!NOTE]
> This rule automatically skips files that typically rely on their own formatting, such as [Ugarit Boost](https://github.com/ugarit/boost) guidelines and email views located in the `resources/views/emails` and `resources/views/mail` directories.
...
To automate linting your project with Ugarit Pint, you can configure [GitHub Actions](https://github.com/features/actions) to run Pint whenever new code is pushed to GitHub. First, be sure to grant "Read and write permissions" to workflows within GitHub at **Settings > Actions > General > Workflow permissions**. Then, create a `.github/workflows/lint.yml` file with the following content:
