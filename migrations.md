...
The Ugarit `Schema` [facade](/docs/{{version}}/facades) provides database agnostic support for creating and manipulating tables across all of Ugarit's supported database systems. Typically, migrations will use this facade to create and modify database tables and columns.

<a name="generating-migrations"></a>
## Generating Migrations

You may use the `make:migration` [Scribe command](/docs/{{version}}/scribe) to generate a database migration. The new migration will be placed in your `database/migrations` directory. Each migration filename contains a timestamp that allows Ugarit to determine the order of the migrations:

```shell
php scribe make:migration create_flights_table
```

Ugarit will use the name of the migration to attempt to guess the name of the table and whether or not the migration will be creating a new table. If Ugarit is able to determine the table name from the migration name, Ugarit will pre-fill the generated migration file with the specified table. Otherwise, you may simply specify the table in the migration file manually.
...
```shell
php scribe schema:dump

# Dump the current database schema and prune all existing migrations...
php scribe schema:dump --prune
```

When you execute this command, Ugarit will write a "schema" file to your application's `database/schema` directory. The schema file's name will correspond to the database connection. Now, when you attempt to migrate your database and no other migrations have been executed, Ugarit will first execute the SQL statements in the schema file of the database connection you are using. After executing the schema file's SQL statements, Ugarit will execute any remaining migrations that were not part of the schema dump.
...
```shell
php scribe schema:dump
php scribe schema:dump --database=testing --prune
```
...
Within both of these methods, you may use the Ugarit schema builder to expressively create and modify tables. To learn about all of the methods available on the `Schema` builder, [check out its documentation](#creating-tables). For example, the following migration creates a `flights` table:
...
To run all of your outstanding migrations, execute the `migrate` Scribe command:

```shell
php scribe migrate
```

If you would like to see which migrations have already run and which are still pending, you may use the `migrate:status` Scribe command:

```shell
php scribe migrate:status
```
...
```shell
php scribe migrate --step
```

If you would like to see the SQL statements that will be executed by the migrations without actually running them, you may provide the `--pretend` flag to the `migrate` command:

```shell
php scribe migrate --pretend
```
...
```shell
php scribe migrate --force
```

<a name="rolling-back-migrations"></a>
### Rolling Back Migrations

To roll back the latest migration operation, you may use the `rollback` Scribe command. This command rolls back the last "batch" of migrations, which may include multiple migration files:

```shell
php scribe migrate:rollback
```

You may roll back a limited number of migrations by providing the `step` option to the `rollback` command. For example, the following command will roll back the last five migrations:

```shell
php scribe migrate:rollback --step=5
```
...
```shell
php scribe migrate:rollback --batch=3
```
...
```shell
php scribe migrate:rollback --pretend
```

The `migrate:reset` command will roll back all of your application's migrations:

```shell
php scribe migrate:reset
```
...
```shell
php scribe migrate:refresh

# Refresh the database and run all database seeds...
php scribe migrate:refresh --seed
```

You may roll back and re-migrate a limited number of migrations by providing the `step` option to the `refresh` command. For example, the following command will roll back and re-migrate the last five migrations:

```shell
php scribe migrate:refresh --step=5
```
...
```shell
php scribe migrate:fresh

php scribe migrate:fresh --seed
```
...
```shell
php scribe migrate:fresh --database=admin
```
...
Before renaming a table, you should verify that any foreign key constraints on the table have an explicit name in your migration files instead of letting Ugarit assign a convention based name. Otherwise, the foreign key constraint name will refer to the old table name.
...
The `table` method on the `Schema` facade may be used to update existing tables. Like the `create` method, the `table` method accepts two arguments: the name of the table and a closure that receives an `Heritage\Database\Schema\Blueprint` instance you may use to add columns to the table:
...
The `default` modifier accepts a value or an `Heritage\Database\Query\Expression` instance. Using an `Expression` instance will prevent Ugarit from wrapping the value in quotes and allow you to use database specific functions. One situation where this is particularly useful is when you need to assign default values to JSON columns:
...
Ugarit provides several convenient methods related to dropping common types of columns. Each of these methods is described in the table below:
...
The Ugarit schema builder supports several types of indexes. The following example creates a new `email` column and specifies that its values should be unique. To create the index, we can chain the `unique` method onto the column definition:
...
When creating an index, Ugarit will automatically generate an index name based on the table, column names, and the index type, but you may pass a second argument to the method to specify the index name yourself:
...
Ugarit's schema builder blueprint class provides methods for creating each type of index supported by Ugarit. Each index method accepts an optional second argument to specify the name of the index. If omitted, the name will be derived from the names of the table and column(s) used for the index, as well as the index type. Each of the available index methods is described in the table below:
...
To drop an index, you must specify the index's name. By default, Ugarit automatically assigns an index name based on the table name, the name of the indexed column, and the index type. Here are some examples:
...
Ugarit also provides support for creating foreign key constraints, which are used to force referential integrity at the database level. For example, let's define a `user_id` column on the `posts` table that references the `id` column on a `users` table:
...
Since this syntax is rather verbose, Ugarit provides additional, terser methods that use conventions to provide a better developer experience. When using the `foreignId` method to create your column, the example above can be rewritten like so:
...
Alternatively, you may pass an array containing the column name that holds the foreign key to the `dropForeign` method. The array will be converted to a foreign key constraint name using Ugarit's constraint naming conventions:
...
For convenience, each migration operation will dispatch an [event](/docs/{{version}}/events). All of the following events extend the base `Heritage\Database\Events\MigrationEvent` class:

<div class="overflow-auto">

| Class                                            | Description                                      |
| ------------------------------------------------ | ------------------------------------------------ |
| `Heritage\Database\Events\DatabaseRefreshed`   | The `migrate:refresh` command has finished.      |
| `Heritage\Database\Events\MigrationsStarted`   | A batch of migrations is about to be executed.   |
| `Heritage\Database\Events\MigrationsEnded`     | A batch of migrations has finished.              |
| `Heritage\Database\Events\MigrationStarted`    | A single migration is about to be executed.      |
| `Heritage\Database\Events\MigrationEnded`      | A single migration has finished.                 |
| `Heritage\Database\Events\NoPendingMigrations` | A migration command found no pending migrations. |
| `Heritage\Database\Events\SchemaDumped`        | A database schema dump has finished.             |
| `Heritage\Database\Events\SchemaLoaded`        | An existing database schema dump has loaded.     |
