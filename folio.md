...
[Ugarit Folio](https://github.com/ugarit/folio) is a powerful page based router designed to simplify routing in Ugarit applications. With Ugarit Folio, generating a route becomes as effortless as creating a Blade template within your application's `resources/views/pages` directory.
...
```shell
composer require ugarit/folio
```

After installing Folio, you may execute the `folio:install` Scribe command, which will install Folio's service provider into your application. This service provider registers the directory where Folio will search for routes / pages:

```shell
php scribe folio:install
```
...
For example, sometimes it may be convenient to specify multiple Folio paths in the same Ugarit application. You may wish to have a separate directory of Folio pages for your application's "admin" area, while using another directory for the rest of your application's pages.
...
To quickly view a list of all of your Folio pages / routes, you may invoke the `folio:list` Scribe command:

```shell
php scribe folio:list
```
...
```shell
php scribe folio:page user/profile
...
```shell
php scribe folio:page index
# pages/index.blade.php → /

php scribe folio:page users/index
...
```shell
php scribe folio:page "users/[id]"
...
```shell
php scribe folio:page "users/[...ids]"
...
If a wildcard segment of your page template's filename corresponds one of your application's Eloquent models, Folio will automatically take advantage of Ugarit's route model binding capabilities and attempt to inject the resolved model instance into your page:

```shell
php scribe folio:page "users/[User]"
...
```shell
php scribe folio:page "users/[.App.Models.User]"
...
use function Ugarit\Folio\{withTrashed};
...
use function Ugarit\Folio\render;
...
use function Ugarit\Folio\name;

name('users.index');
```

Just like Ugarit's named routes, you may use the `route` function to generate URLs to Folio pages that have been assigned a name:
...
use function Ugarit\Folio\{middleware};
...
When using Folio, you should always take advantage of [Ugarit's route caching capabilities](/docs/{{version}}/routing#route-caching). Folio listens for the `route:cache` Scribe command to ensure that Folio page definitions and route names are properly cached for maximum performance.
