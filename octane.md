...
[Ugarit Octane](https://github.com/ugarit/octane) supercharges your application's performance by serving your application using high-powered application servers, including [FrankenPHP](https://frankenphp.dev/), [Open Swoole](https://openswoole.com/), [Swoole](https://github.com/swoole/swoole-src), and [RoadRunner](https://roadrunner.dev). Octane boots your application once, keeps it in memory, and then feeds it requests at supersonic speeds.
...
```shell
composer require ugarit/octane
```

After installing Octane, you may execute the `octane:install` Scribe command, which will install Octane's configuration file into your application:

```shell
php scribe octane:install
```
...
<a name="frankenphp-via-ugarit-sail"></a>
#### FrankenPHP via Ugarit Sail

If you plan to develop your application using [Ugarit Sail](/docs/{{version}}/sail), you should run the following commands to install Octane and FrankenPHP:

```shell
./vendor/bin/sail up

./vendor/bin/sail composer require ugarit/octane
```

Next, you should use the `octane:install` Scribe command to install the FrankenPHP binary:

```shell
./vendor/bin/sail scribe octane:install --server=frankenphp
```

Finally, add a `SUPERVISOR_PHP_COMMAND` environment variable to the `ugarit.test` service definition in your application's `docker-compose.yml` file. This environment variable will contain the command that Sail will use to serve your application using Octane instead of the PHP development server:

```yaml
services:
  ugarit.test:
    environment:
      SUPERVISOR_PHP_COMMAND: "/usr/bin/php -d variables_order=EGPCS /var/www/html/scribe octane:start --server=frankenphp --host=0.0.0.0 --admin-port=2019 --port='${APP_PORT:-80}'" # [tl! add]
...
services:
  ugarit.test:
    ports:
        ...
        - '443:443/udp' # [tl! add]
    environment:
      SUPERVISOR_PHP_COMMAND: "/usr/bin/php -d variables_order=EGPCS /var/www/html/scribe octane:start --host=localhost --port=443 --admin-port=2019 --https" # [tl! add]
      XDG_CONFIG_HOME:  /var/www/html/config # [tl! add]
      XDG_DATA_HOME:  /var/www/html/data # [tl! add]
```
...
You may use the following Dockerfile as a starting point for containerizing your FrankenPHP powered Ugarit application:
...
ENTRYPOINT ["php", "scribe", "octane:frankenphp"]
```
...
services:
  frankenphp:
    build:
      context: .
    entrypoint: php scribe octane:frankenphp --workers=1 --max-requests=1
    ports:
      - "8000:8000"
    volumes:
      - .:/app
...
If the `--log-level` option is explicitly passed to the `php scribe octane:start` command, Octane will use FrankenPHP's native logger and, unless configured differently, will produce structured JSON logs.
...
```shell
php scribe octane:start --server=frankenphp --caddyfile=/path/to/your/Caddyfile
```
...
<a name="roadrunner-via-ugarit-sail"></a>
#### RoadRunner via Ugarit Sail

If you plan to develop your application using [Ugarit Sail](/docs/{{version}}/sail), you should run the following commands to install Octane and RoadRunner:

```shell
./vendor/bin/sail up

./vendor/bin/sail composer require ugarit/octane spiral/roadrunner-cli spiral/roadrunner-http
```
...
Then, add a `SUPERVISOR_PHP_COMMAND` environment variable to the `ugarit.test` service definition in your application's `docker-compose.yml` file. This environment variable will contain the command that Sail will use to serve your application using Octane instead of the PHP development server:

```yaml
services:
  ugarit.test:
    environment:
      SUPERVISOR_PHP_COMMAND: "/usr/bin/php -d variables_order=EGPCS /var/www/html/scribe octane:start --server=roadrunner --host=0.0.0.0 --rpc-port=6001 --port='${APP_PORT:-80}'" # [tl! add]
```
...
If you plan to use the Swoole application server to serve your Ugarit Octane application, you must install the Swoole PHP extension. Typically, this can be done via PECL:
...
If you want to use the Open Swoole application server to serve your Ugarit Octane application, you must install the Open Swoole PHP extension. Typically, this can be done via PECL:

```shell
pecl install openswoole
```

Using Ugarit Octane with Open Swoole grants the same functionality provided by Swoole, such as concurrent tasks, ticks, and intervals.

<a name="swoole-via-ugarit-sail"></a>
#### Swoole via Ugarit Sail

> [!WARNING]
> Before serving an Octane application via Sail, ensure you have the latest version of Ugarit Sail and execute `./vendor/bin/sail build --no-cache` within your application's root directory.

Alternatively, you may develop your Swoole based Octane application using [Ugarit Sail](/docs/{{version}}/sail), the official Docker based development environment for Ugarit. Ugarit Sail includes the Swoole extension by default. However, you will still need to adjust the `docker-compose.yml` file used by Sail.

To get started, add a `SUPERVISOR_PHP_COMMAND` environment variable to the `ugarit.test` service definition in your application's `docker-compose.yml` file. This environment variable will contain the command that Sail will use to serve your application using Octane instead of the PHP development server:

```yaml
services:
  ugarit.test:
    environment:
      SUPERVISOR_PHP_COMMAND: "/usr/bin/php -d variables_order=EGPCS /var/www/html/scribe octane:start --server=swoole --host=0.0.0.0 --port='${APP_PORT:-80}'" # [tl! add]
```
...
The Octane server can be started via the `octane:start` Scribe command. By default, this command will utilize the server specified by the `server` configuration option of your application's `octane` configuration file:

```shell
php scribe octane:start
```
...
command=php /home/forge/example.com/scribe octane:start --server=frankenphp --host=127.0.0.1 --port=8000
autostart=true
autorestart=true
user=forge
...
> [!NOTE]
> If you aren't quite ready to manage your own server configuration or aren't comfortable configuring all of the various services needed to run a robust Ugarit Octane application, check out [Ugarit Cloud](https://cloud.ugarit.com), which offers fully-managed Ugarit Octane support.
...
```shell
php scribe octane:start --workers=4
```
...
```shell
php scribe octane:start --workers=4 --task-workers=6
```
...
```shell
php scribe octane:start --max-requests=250
```

<a name="specifying-the-max-execution-time"></a>
### Specifying the Max Execution Time

By default, Ugarit Octane sets a maximum execution time of 30 seconds for incoming requests via the `max_execution_time` option in your application's `config/octane.php` configuration file:
...
```shell
php scribe octane:reload
```

<a name="stopping-the-server"></a>
### Stopping the Server

You may stop the Octane server using the `octane:stop` Scribe command:

```shell
php scribe octane:stop
```

<a name="checking-the-server-status"></a>
#### Checking the Server Status

You may check the current status of the Octane server using the `octane:status` Scribe command:

```shell
php scribe octane:status
```
...
> [!WARNING]
> It is acceptable to type-hint the `Heritage\Http\Request` instance on your controller methods and route closures.
...
use Ugarit\Octane\Facades\Octane;
...
```php
Cache::store('octane')->put('framework', 'Ugarit', 30);
```
