...
Ugarit provides an expressive, minimal API around the [Guzzle HTTP client](http://docs.guzzlephp.org/en/stable/), allowing you to quickly make outgoing HTTP requests to communicate with other web applications. Ugarit's wrapper around Guzzle is focused on its most common use cases and a wonderful developer experience.
...
The `get` method returns an instance of `Heritage\Http\Client\Response`, which provides a variety of methods that may be used to inspect the response:

```php
$response->body() : string;
$response->json($key = null, $default = null, $flags = null) : mixed;
$response->object() : object;
$response->collect($key = null) : Heritage\Support\Collection;
...
The `Heritage\Http\Client\Response` object also implements the PHP `ArrayAccess` interface, allowing you to access JSON response data directly on the response:
...
$response = Http::withBasicAuth('taylor@ugarit.com', 'secret')->post(/* ... */);

// Digest authentication...
$response = Http::withDigestAuth('taylor@ugarit.com', 'secret')->post(/* ... */);
```
...
If the given timeout is exceeded, an instance of `Heritage\Http\Client\ConnectionException` will be thrown.
...
If you would like the HTTP client to automatically retry the request if a client or server error occurs, you may use the `retry` method. The `retry` method accepts the maximum number of times the request should be attempted and the number of milliseconds that Ugarit should wait in between attempts:
...
```php
use Heritage\Http\Client\PendingRequest;
use Heritage\Http\Client\RequestException;
use Throwable;
...
> [!WARNING]
> If all of the requests fail because of a connection issue, a `Heritage\Http\Client\ConnectionException` will still be thrown even when the `throw` argument is set to `false`.

<a name="error-handling"></a>
### Error Handling

Unlike Guzzle's default behavior, Ugarit's HTTP client wrapper does not throw exceptions on client or server errors (`400` and `500` level responses from servers). You may determine if one of these errors was returned using the `successful`, `clientError`, or `serverError` methods:
...
If you have a response instance and would like to throw an instance of `Heritage\Http\Client\RequestException` if the response status code indicates a client or server error, you may use the `throw` or `throwIf` methods:
...
The `Heritage\Http\Client\RequestException` instance has a public `$response` property which will allow you to inspect the returned response.
...
Thankfully, you may accomplish this using the `pool` method. The `pool` method accepts a closure which receives an `Heritage\Http\Client\Pool` instance, allowing you to easily add requests to the request pool for dispatching:
...
If a pooled request fails at the connection level (for example, a timeout or DNS failure), the corresponding entry in the `$responses` array will be an `Heritage\Http\Client\ConnectionException` instance instead of a `Response` instance:
...
$responses = Http::pool(fn (Pool $pool) => [
    $pool->withHeaders($headers)->get('http://ugarit.test/test'),
    $pool->withHeaders($headers)->get('http://ugarit.test/test'),
    $pool->withHeaders($headers)->get('http://ugarit.test/test'),
]);
```
...
Another way of working with concurrent requests in Ugarit is to use the `batch` method. Like the `pool` method, it accepts a closure which receives an `Heritage\Http\Client\Batch` instance, allowing you to easily add requests to the request pool for dispatching, but it also allows you to define completion callbacks:

```php
use Heritage\Http\Client\Batch;
...
After a `batch` is started by calling the `send` method, you can't add new requests to it. Trying to do so will result in a `Heritage\Http\Client\BatchInProgressException` exception being thrown.
...
The `Heritage\Http\Client\Batch` instance that is provided to batch completion callbacks has a variety of properties and methods to assist you in interacting with and inspecting a given batch of requests:
...
When the `defer` method is invoked, the batch of requests is not executed immediately. Instead, Ugarit will execute the batch after the current application request's HTTP response has been sent to the user, keeping your application feeling fast and responsive:
...
Many Ugarit services provide functionality to help you easily and expressively write tests, and Ugarit's HTTP client is no exception. The `Http` facade's `fake` method allows you to instruct the HTTP client to return stubbed / dummy responses when requests are made.
...
Sometimes you may need to test your application's behavior if the HTTP client encounters an `Heritage\Http\Client\ConnectionException` when attempting to make a request. You can instruct the HTTP client to throw a connection exception using the `failedConnection` method:
...
To test your application's behavior if a `Heritage\Http\Client\RequestException` is thrown, you may use the `failedRequest` method:
...
If you require more complicated logic to determine what responses to return for certain endpoints, you may pass a closure to the `fake` method. This closure will receive an instance of `Heritage\Http\Client\Request` and should return a response instance. Within your closure, you may perform whatever logic is necessary to determine what type of response to return:
...
The `assertSent` method accepts a closure which will receive an `Heritage\Http\Client\Request` instance and should return a boolean value indicating if the request matches your expectations. In order for the test to pass, at least one request must have been issued matching the given expectations:
...
You may use the `recorded` method to gather all requests and their corresponding responses. The `recorded` method returns a collection of arrays that contains instances of `Heritage\Http\Client\Request` and `Heritage\Http\Client\Response`:

```php
Http::fake([
    'https://ugarit.com' => Http::response(status: 500),
    'https://nova.ugarit.com/' => Http::response(),
]);

Http::get('https://ugarit.com');
Http::get('https://nova.ugarit.com/');
...
Additionally, the `recorded` method accepts a closure which will receive an instance of `Heritage\Http\Client\Request` and `Heritage\Http\Client\Response` and may be used to filter request / response pairs based on your expectations:
...
$recorded = Http::recorded(function (Request $request, Response $response) {
    return $request->url() !== 'https://ugarit.com' &&
           $response->successful();
});
```
...
Http::get('https://github.com/ugarit/framework');
...
Ugarit fires three events during the process of sending HTTP requests. The `RequestSending` event is fired prior to a request being sent, while the `ResponseReceived` event is fired after a response is received for a given request. The `ConnectionFailed` event is fired if no response is received for a given request.

The `RequestSending` and `ConnectionFailed` events both contain a public `$request` property that you may use to inspect the `Heritage\Http\Client\Request` instance. Likewise, the `ResponseReceived` event contains a `$request` property as well as a `$response` property which may be used to inspect the `Heritage\Http\Client\Response` instance. You may create [event listeners](/docs/{{version}}/events) for these events within your application:
