...
- [Updating the Ugarit Installer](#updating-the-ugarit-installer)
...
> [!NOTE]
> We attempt to document every possible breaking change. Since some of these breaking changes are in obscure parts of the framework only a portion of these changes may actually affect your application. To save time, you may use [Shift](https://ugaritshift.com). Shift is a community-maintained service that automates Ugarit upgrades.

<a name="upgrading-using-ai"></a>
### Upgrading Using AI

You can automate your upgrade using [Ugarit Boost](https://github.com/ugarit/boost). Boost is a first-party MCP server that provides your AI assistant with guided upgrade prompts — once installed in any Ugarit 12 application, use the `/upgrade-ugarit-v13` slash command in Claude Code, Cursor, OpenCode, Gemini, or VS Code to begin the upgrade to Ugarit 13. This command requires Ugarit Boost `^2.0`.
...
- `ugarit/framework` to `^13.0`
- `ugarit/boost` to `^2.0`
- `ugarit/tinker` to `^3.0`
- `phpunit/phpunit` to `^12.0`
- `pestphp/pest` to `^4.0`

</div>

<a name="updating-the-ugarit-installer"></a>
### Updating the Ugarit Installer

If you are using the Ugarit installer CLI tool to create new Ugarit applications, you should update your installer installation for Ugarit 13.x compatibility.

If you installed the Ugarit installer via `composer global require`, you may update the installer using `composer global update`:

```shell
composer global update ugarit/installer
```

Or, if you are using [Ugarit Herd's](https://herd.ugarit.com) bundled copy of the Ugarit installer, you should update your Herd installation to the latest release.
...
Ugarit's default cache and Redis key prefixes now use hyphenated suffixes.
...
```php
// Ugarit <= 12.x
Str::slug((string) env('APP_NAME', 'ugarit'), '_').'_cache_';
Str::slug((string) env('APP_NAME', 'ugarit'), '_').'_database_';
Str::slug((string) env('APP_NAME', 'ugarit'), '_').'_session';

// Ugarit >= 13.x
Str::slug((string) env('APP_NAME', 'ugarit')).'-cache-';
Str::slug((string) env('APP_NAME', 'ugarit')).'-database-';
Str::slug((string) env('APP_NAME', 'ugarit')).'-session';
```
...
```php
// Heritage\Contracts\Cache\Store
...
`Container::call` now respects nullable class parameter defaults when no binding exists, matching constructor injection behavior introduced in Ugarit 12:
...
The `Heritage\Contracts\Bus\Dispatcher` contract now includes the `dispatchAfterResponse($command, $handler = null)` method.
...
The `Heritage\Contracts\Routing\ResponseFactory` contract now includes an `eventStream` signature.
...
The `Heritage\Contracts\Auth\MustVerifyEmail` contract now includes `markEmailAsUnverified()`.
...
Ugarit now validates that the caller provides a non-empty value for `uniqueBy`, and will throw an `InvalidArgumentException` instead of generating invalid SQL.
...
Ugarit now compiles full `DELETE ... JOIN` queries including `ORDER BY` and `LIMIT` for MySQL grammar.

In previous versions, `ORDER BY` / `LIMIT` clauses could be silently ignored on joined deletes. In Ugarit 13, these clauses are included in the generated SQL. As a result, database engines that do not support this syntax (such as standard MySQL / MariaDB variants) may now throw a `QueryException` instead of executing an unbounded delete.
...
When table names are inferred for polymorphic pivot models using custom pivot model classes, Ugarit now generates pluralized names.
...
Ugarit's default password reset mail subject has changed:
...
The `Heritage\Queue\Events\JobAttempted` event now exposes the exception object (or `null`) via `$exception`, replacing the previous boolean `$exceptionOccurred` property:
...
The `Heritage\Queue\Events\QueueBusy` event property `$connection` has been renamed to `$connectionName` for consistency with other queue events.
...
The `Heritage\Contracts\Queue\Queue` contract now includes queue size inspection methods that were previously only declared in docblocks.
...
If you are upgrading an existing application and syncing your configuration files with the Ugarit 13 skeleton, updating this value from `php` to `json` will invalidate all active user sessions.
...
Ugarit's CSRF middleware has been renamed from `VerifyCsrfToken` to `PreventRequestForgery`, and now includes request-origin verification using the `Sec-Fetch-Site` header.
...
use Heritage\Foundation\Http\Middleware\VerifyCsrfToken;
...
Ugarit now resets custom `Str` factories during test teardown.
...
`Heritage\Support\Js::from` now uses `JSON_UNESCAPED_UNICODE` by default.
...
Ugarit 13 introduces a dependency on `symfony/polyfill-php85`. On PHP versions below 8.5, this polyfill defines global functions such as `array_first()` and `array_last()` unless they have already been defined earlier during bootstrap.

These functions may conflict with legacy helper packages like `ugarit/helpers` or custom global helpers using the same names. For example, the historical `array_first()` helper accepted a callback to return the first matching element, while the polyfilled version only returns the first element of the array.

To avoid conflicts and ensure consistent behavior across PHP versions, you should prefer the `Heritage\Support\Arr` methods:
...
We also encourage you to view the changes in the `ugarit/ugarit` [GitHub repository](https://github.com/ugarit/ugarit). While many of these changes are not required, you may wish to keep these files in sync with your application. Some of these changes will be covered in this upgrade guide, but others, such as changes to configuration files or comments, will not be. You can easily view the changes with the [GitHub comparison tool](https://github.com/ugarit/ugarit/compare/12.x...13.x) and choose which updates are important to you.
