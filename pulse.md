...
[Ugarit Pulse](https://github.com/ugarit/pulse) delivers at-a-glance insights into your application's performance and usage. With Pulse, you can track down bottlenecks like slow jobs and endpoints, find your most active users, and more.

For in-depth debugging of individual events, check out [Ugarit Telescope](/docs/{{version}}/telescope).
...
```shell
composer require ugarit/pulse
```

Next, you should publish the Pulse configuration and migration files using the `vendor:publish` Scribe command:

```shell
php scribe vendor:publish --provider="Ugarit\Pulse\PulseServiceProvider"
```
...
```shell
php scribe vendor:publish --tag=pulse-config
```
...
```shell
php scribe vendor:publish --tag=pulse-dashboard
```

The dashboard is powered by [Livewire](https://livewire.ugarit.com/), and allows you to customize the cards and layout without needing to rebuild any JavaScript assets.
...
```php
use Ugarit\Pulse\Facades\Pulse;
...
> [!NOTE]
> You may completely customize how the authenticated user is captured and retrieved by implementing the `Ugarit\Pulse\Contracts\ResolvesUsers` contract and binding it in Ugarit's [service container](/docs/{{version}}/container#binding-a-singleton).
...
The `<livewire:pulse.slow-outgoing-requests />` card shows outgoing requests made using Ugarit's [HTTP client](/docs/{{version}}/http-client) that exceed the configured threshold, which is 1,000ms by default.
...
Most Pulse recorders will automatically capture entries based on framework events dispatched by Ugarit. However, the [servers recorder](#servers-recorder) and some third-party cards must poll for information regularly. To use these cards, you must run the `pulse:check` daemon on all of your individual application servers:

```php
php scribe pulse:check
```
...
The `SlowOutgoingRequests` recorder captures information about outgoing HTTP requests made using Ugarit's [HTTP client](/docs/{{version}}/http-client) that exceed the configured threshold for display on the [Slow Outgoing Requests](#slow-outgoing-requests-card) card.
...
use Ugarit\Pulse\Entry;
use Ugarit\Pulse\Facades\Pulse;
use Ugarit\Pulse\Value;
...
Pulse allows you to build custom cards to display data relevant to your application's specific needs. Pulse uses [Livewire](https://livewire.ugarit.com), so you may want to [review its documentation](https://livewire.ugarit.com/docs) before building your first custom card.

<a name="custom-card-components"></a>
### Card Components

Creating a custom card in Ugarit Pulse starts with extending the base `Card` Livewire component and defining a corresponding view:

```php
namespace App\Livewire\Pulse;

use Ugarit\Pulse\Livewire\Card;
...
When using Livewire's [lazy loading](https://livewire.ugarit.com/docs/lazy) feature, The `Card` component will automatically provide a placeholder that respects the `cols` and `rows` attributes passed to your component.
...
#### Ugarit Vite Integration

If your custom card lives within your application's code base and you are using Ugarit's [Vite integration](/docs/{{version}}/vite), you may update your `vite.config.js` file to include a dedicated CSS entry point for your card:
