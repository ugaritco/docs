...
[Ugarit Head](https://github.com/ugarit/head) provides a fluent API for managing your application's document `<head>` element, including title and meta tags, Open Graph metadata, canonical URLs, robots directives, performance hints, and structured data. It works with Blade, Livewire, and Inertia.

<a name="installation"></a>
## Installation

You may install Ugarit Head using the Composer package manager:

```shell
composer require ugarit/head
```
...
```php
use Ugarit\Head\Facades\Head;
use Ugarit\Head\HeadBuilder;

Head::defaults(fn (HeadBuilder $head) => $head
    ->title('Ugarit', suffix: ' - Ugarit')
...
Ugarit Head allows you to define metadata using site-wide defaults, route metadata, runtime calls, and error page definitions.
...
Head::defaults(function (HeadBuilder $head) {
    $head
        ...
        ->description('Build something great.')
        ->canonical()
        ->og(siteName: 'Ugarit', type: OgType::Website)
        ->searchableByRobots()
        ->preconnect('https://fonts.example.com');
});
```

Defaults are the lowest-priority page metadata layer. If no route, runtime, or error metadata sets a title, `Ugarit` renders as-is. When a higher layer sets a page title, the inherited suffix is applied, so `Head::title('About')` renders `About - Ugarit`. Pass `exact: true` for titles that should ignore an inherited prefix or suffix.
...
The `withHead` method stores plain arrays through Ugarit's native route metadata API. It is equivalent to calling the `metadata` method with the attributes nested under a `head` key, so the metadata remains compatible with cached routes.

The named arguments are intentionally limited to Ugarit Head's built-in route properties so editors and static analysis can catch misspelled names. Route attributes registered by custom tag builders may be passed through `extensions`:
...
Ugarit automatically detects the response status when rendering an error view or executing a respond-phase hook such as Inertia's `handleExceptionsUsing()` method. If you render an error response inside an `$exceptions->render()` callback, call `Head::status(404)` before rendering so the error metadata is applied.
...
```php
use Ugarit\Head\Enums\ImageType;
use Ugarit\Head\Enums\OgType;
...
```php
use Ugarit\Head\Enums\TwitterCard;
use Ugarit\Head\Facades\Head;
use Ugarit\Head\HeadBuilder;
...
```php
Head::title('Introducing Ugarit Head')
    ->description('A fluent API for Ugarit document head metadata.')
    ->ogImage('https://example.com/social.jpg', alt: 'Introducing Ugarit Head');
```
...
<meta name="twitter:title" content="Introducing Ugarit Head">
<meta name="twitter:description" content="A fluent API for Ugarit document head metadata.">
<meta name="twitter:image" content="https://example.com/social.jpg">
<meta name="twitter:image:alt" content="Introducing Ugarit Head">
```
...
Ugarit Head includes methods for common browser and application metadata:

```php
use Ugarit\Head\Enums\ImageType;
use Ugarit\Head\Enums\Media;

Head::applicationName('Ugarit')
    ->colorScheme('light dark')
    ->referrer('strict-origin-when-cross-origin')
    ->viewport('width=device-width, initial-scale=1')
    ->appleWebAppTitle('Ugarit')
    ->webAppCapable()
...
Ugarit Head renders performance hints, pagination links, locale alternates, and feed discovery:
...
Head::preload(asset('fonts/inter.woff2'), as: 'font', crossorigin: true)
    ...
    ->feed('/feed', title: 'Ugarit RSS')
    ->feed('/feed.atom', type: 'atom', title: 'Ugarit Atom');
```
...
Head::meta('format-detection', 'telephone=no')
    ...
    ->link('me', 'https://social.example.com/@ugarit');
```
...
```php
Head::meta('description', 'About Ugarit')
    ->meta('og:title', 'About Ugarit');
```

```html
<meta name="description" content="About Ugarit">
<meta property="og:title" content="About Ugarit">
```
...
```php
use Ugarit\Head\Enums\OfferAvailability;
use Ugarit\Head\Facades\Schema;
...
When JSON-LD schema data is invalid, Ugarit Head throws an exception in non-production environments and logs a warning in production.
...
Head::schema(
    Schema::faq()->questions([
        'What is Ugarit Head?' => 'A fluent API for managing the document head.',
...
use Ugarit\Head\Facades\Schema;
use Ugarit\Head\Schema\SchemaObject;
use Ugarit\Head\SchemaType;
...
Ugarit Head resolves page metadata into tags for the current response. How these tags are rendered depends on your application stack.

The HTML renderer powers the `@head` directive and the rendered elements that Ugarit Head shares with Inertia via the `head` prop. The array renderer powers `Head::toArray()` for applications that need the resolved metadata as structured data.
...
When Inertia is installed, Ugarit Head automatically shares the page-managed head as an array of rendered element strings under a `head` prop on every page object:

```json
{
    "props": {
        "head": [
            "<title data-inertia=\"title\">Dashboard - Ugarit</title>",
...
This works with or without [server-side rendering (SSR)](https://inertiajs.com/docs/v3/advanced/server-side-rendering). If your application has a separate SSR entry point, enable `serverHead` there too. Ugarit Head automatically deduplicates page-managed elements between `@head` and `<x-inertia::head />`, regardless of their order, while preserving other head elements produced by JavaScript SSR.

> [!NOTE]
> When adding Ugarit Head to an existing Inertia application, remove any title callbacks from `resources/js/app.tsx` and `resources/js/ssr.tsx` so Ugarit Head can manage the final document title, and move tags managed by Inertia's [`<Head>` component](https://inertiajs.com/docs/v3/the-basics/title-and-meta) into Ugarit Head so the two never define the same element.
...
Most tags should live in defaults, route metadata, or runtime metadata so Ugarit Head can resolve the right value for each page. Use Inertia globals only for document tags rendered in the first HTML response and left unchanged by Inertia for the rest of the session.
