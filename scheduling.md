...
- [Defining Schedules](#defining-schedules)
    - [Scheduling Scribe Commands](#scheduling-scribe-commands)
...
Ugarit's command scheduler offers a fresh approach to managing scheduled tasks on your server. The scheduler allows you to fluently and expressively define your command schedule within your Ugarit application itself. When using the scheduler, only a single cron entry is needed on your server. Your task schedule is typically defined in your application's `routes/console.php` file.
...
If you would like to view an overview of your scheduled tasks and the next time they are scheduled to run, you may use the `schedule:list` Scribe command:

```shell
php scribe schedule:list
```

<a name="scheduling-scribe-commands"></a>
### Scheduling Scribe Commands

In addition to scheduling closures, you may also schedule [Scribe commands](/docs/{{version}}/scribe) and system commands. For example, you may use the `command` method to schedule an Scribe command using either the command's name or class.

When scheduling Scribe commands using the command's class name, you may pass an array of additional command-line arguments that should be provided to the command when it is invoked:
...
<a name="scheduling-scribe-closure-commands"></a>
#### Scheduling Scribe Closure Commands

If you want to schedule an Scribe command defined by a closure, you may chain the scheduling related methods after the command's definition:

```php
Scribe::command('delete:recent-users', function () {
    DB::table('recent_users')->delete();
...
```php
Scribe::command('emails:send {user} {--force}', function ($user) {
    // ...
...
Alternatively, you may use the constants available on the `Heritage\Console\Scheduling\Schedule` class when defining the days on which a task should run:

```php
use Heritage\Support\Facades;
use Heritage\Console\Scheduling\Schedule;
...
In this example, the `emails:send` [Scribe command](/docs/{{version}}/scribe) will be run every minute if it is not already running. The `withoutOverlapping` method is especially useful if you have tasks that vary drastically in their execution time, preventing you from predicting exactly how long a given task will take.
...
Behind the scenes, the `withoutOverlapping` method utilizes your application's [cache](/docs/{{version}}/cache) to obtain locks. If necessary, you can clear these cache locks using the `schedule:clear-cache` Scribe command. This is typically only necessary if a task becomes stuck due to an unexpected server problem.
...
Sometimes you may need to schedule the same job to be dispatched with different parameters, while still instructing Ugarit to run each permutation of the job on a single server. To accomplish this, you may assign each schedule definition a unique name via the `name` method:

```php
Schedule::job(new CheckUptime('https://ugarit.com'))
    ->name('check_uptime:ugarit.com')
    ->everyFiveMinutes()
    ->onOneServer();

Schedule::job(new CheckUptime('https://vapor.ugarit.com'))
    ->name('check_uptime:vapor.ugarit.com')
    ->everyFiveMinutes()
    ->onOneServer();
```
...
You may temporarily pause scheduled task processing without changing your deployed code by using the `schedule:pause` Scribe command:

```shell
php scribe schedule:pause
```
...
When defining multiple scheduled tasks with similar configurations, you can use Ugarit's task grouping feature to avoid repeating the same settings for each task. Grouping tasks simplifies your code and ensures consistency across related tasks.
...
Now that we have learned how to define scheduled tasks, let's discuss how to actually run them on our server. The `schedule:run` Scribe command will evaluate all of your scheduled tasks and determine if they need to run based on the server's current time.

So, when using Ugarit's scheduler, we only need to add a single cron configuration entry to our server that runs the `schedule:run` command every minute. If you do not know how to add cron entries to your server, consider using a managed platform such as [Ugarit Cloud](https://cloud.ugarit.com) which can manage the scheduled task execution for you:

```shell
* * * * * cd /path-to-your-project && php scribe schedule:run >> /dev/null 2>&1
```

<a name="sub-minute-scheduled-tasks"></a>
### Sub-Minute Scheduled Tasks

On most operating systems, cron jobs are limited to running a maximum of once per minute. However, Ugarit's scheduler allows you to schedule tasks to run at more frequent intervals, even as often as once per second:
...
```shell
php scribe schedule:interrupt
```

<a name="running-the-scheduler-locally"></a>
### Running the Scheduler Locally

Typically, you would not add a scheduler cron entry to your local development machine. Instead, you may use the `schedule:work` Scribe command. This command will run in the foreground and invoke the scheduler every minute until you terminate the command. When sub-minute tasks are defined, the scheduler will continue running within each minute to process those tasks:

```shell
php scribe schedule:work
```

<a name="task-output"></a>
## Task Output

The Ugarit scheduler provides several convenient methods for working with the output generated by scheduled tasks. First, using the `sendOutputTo` method, you may send the output to a file for later inspection:
...
Using the `emailOutputTo` method, you may email the output to an email address of your choice. Before emailing the output of a task, you should configure Ugarit's [email services](/docs/{{version}}/mail):
...
If you only want to email the output if the scheduled Scribe or system command terminates with a non-zero exit code, use the `emailOutputOnFailure` method:
...
The `onSuccess` and `onFailure` methods allow you to specify code to be executed if the scheduled task succeeds or fails. A failure indicates that the scheduled Scribe or system command terminated with a non-zero exit code:
...
If output is available from your command, you may access it in your `after`, `onSuccess` or `onFailure` hooks by type-hinting an `Heritage\Support\Stringable` instance as the `$output` argument of your hook's closure definition:
...
The `pingOnSuccess` and `pingOnFailure` methods may be used to ping a given URL only if the task succeeds or fails. A failure indicates that the scheduled Scribe or system command terminated with a non-zero exit code:
...
Ugarit dispatches a variety of [events](/docs/{{version}}/events) during the scheduling process. You may [define listeners](/docs/{{version}}/events) for any of the following events:

<div class="overflow-auto">

| Event Name                                                  |
| ----------------------------------------------------------- |
| `Heritage\Console\Events\ScheduledTaskStarting`           |
| `Heritage\Console\Events\ScheduledTaskFinished`           |
| `Heritage\Console\Events\ScheduledBackgroundTaskFinished` |
| `Heritage\Console\Events\ScheduledTaskSkipped`            |
| `Heritage\Console\Events\ScheduledTaskFailed`             |
