...
[Ugarit Scout](https://github.com/ugarit/scout) provides a simple, driver-based solution for adding full-text search to your [Eloquent models](/docs/{{version}}/eloquent). Using model observers, Scout will automatically keep your search indexes in sync with your Eloquent records.

Scout ships with a built-in `database` engine that uses MySQL / PostgreSQL full-text indexes and `LIKE` clauses to search your existing database — no external service required. For most applications, this is all you need. For an overview of all search options available in Ugarit, consult the [search documentation](/docs/{{version}}/search).
...
```shell
composer require ugarit/scout
```

After installing Scout, you should publish the Scout configuration file using the `vendor:publish` Scribe command. This command will publish the `scout.php` configuration file to your application's `config` directory:

```shell
php scribe vendor:publish --provider="Ugarit\Scout\ScoutServiceProvider"
```

Finally, add the `Ugarit\Scout\Searchable` trait to the model you would like to make searchable. This trait will register a model observer that will automatically keep the model in sync with your search driver:
...
Even when the `queue` option is set to `false`, it's important to remember that some Scout drivers like Algolia and Meilisearch always index records asynchronously. In other words, even though the index operation has completed within your Ugarit application, the search engine itself may not reflect the new and updated records immediately.
...
```shell
php scribe queue:work redis --queue=scout
```
...
```php
use Ugarit\Scout\Jobs\MakeSearchableUniquely;
use Ugarit\Scout\Jobs\RemoveFromSearchUniquely;
use Ugarit\Scout\Scout;

Scout::makeSearchableUsing(MakeSearchableUniquely::class);
Scout::removeFromSearchUsing(RemoveFromSearchUniquely::class);
```

These jobs use Ugarit's [unique job locks](/docs/{{version}}/queues#unique-jobs) to avoid dispatching duplicate queued indexing operations for the same searchable model records while a matching job is already queued.
...
[Meilisearch](https://www.meilisearch.com) is a fast, open source search engine. If you aren't sure how to install Meilisearch on your local machine, you may use [Ugarit Sail](/docs/{{version}}/sail#meilisearch), Ugarit's officially supported Docker development environment.
...
If you are using [Ugarit Sail](/docs/{{version}}/sail), you may need to adjust the `TYPESENSE_HOST` environment variable to match the Docker container name. You may also optionally specify your installation's port, path, and protocol:
...
use Heritage\Database\Eloquent\Model;
use Ugarit\Scout\Engines\Engine;
use Ugarit\Scout\Scout;
use Ugarit\Scout\Searchable;
...
```php
use Ugarit\Scout\Attributes\SearchUsingFullText;
use Ugarit\Scout\Attributes\SearchUsingPrefix;
...
The "collection" engine is intended for quick prototypes, extremely small datasets (a few hundred records), or running tests. It retrieves all possible records from your database and uses Ugarit's `Str::is` helper to filter them in PHP, so it does not require any indexing or database-specific features. For anything beyond trivial use cases, you should use the [database engine](#database-engine) instead.
...
After configuring your application's index settings, you must invoke the `scout:sync-index-settings` Scribe command. This command will inform Algolia of your currently configured index settings. For convenience, you may wish to make this command part of your deployment process:

```shell
php scribe scout:sync-index-settings
```
...
After configuring your application's index settings, you must invoke the `scout:sync-index-settings` Scribe command. This command will inform Meilisearch of your currently configured index settings. For convenience, you may wish to make this command part of your deployment process:
...
The model's `toSearchableEmbedding` method may return source text, which Scout embeds using the [Ugarit AI SDK](/docs/{{version}}/ai-sdk), or a precomputed embedding array. After updating the configuration, run the `scout:sync-index-settings` command.
...
To enable semantic and hybrid search, define an `embedding` setting and vector field in the model's Typesense configuration. By default, Scout uses the [Ugarit AI SDK](/docs/{{version}}/ai-sdk) to generate embeddings:
...
Your model's `toSearchableEmbedding` method should return the source text that Scout should embed or a precomputed embedding array. Scout generates source-text embeddings using the [Ugarit AI SDK](/docs/{{version}}/ai-sdk).

Alternatively, you may use Turbopuffer's native embeddings without installing the Ugarit AI SDK or defining a `toSearchableEmbedding` method. Set the embedding driver to `turbopuffer` and configure an `embed` schema on the searchable source attribute:
...
If you are installing Scout into an existing project, you may already have database records you need to import into your indexes. Scout provides a `scout:import` Scribe command that you may use to import all of your existing records into your search indexes:

```shell
php scribe scout:import "App\Models\Post"
```

The `scout:queue-import` command may be used to import all of your existing records using [queued jobs](/docs/{{version}}/queues):

```shell
php scribe scout:queue-import "App\Models\Post" --chunk=500
```

The `flush` command may be used to remove all of a model's records from your search indexes:

```shell
php scribe scout:flush "App\Models\Post"
```
...
Once you have added the `Ugarit\Scout\Searchable` trait to a model, all you need to do is `save` or `create` a model instance and it will automatically be added to your search index. If you have configured Scout to [use queues](#queueing) this operation will be performed in the background by your queue worker:
...
The database, Meilisearch, Typesense, and Turbopuffer engines support semantic search, which matches records based on the meaning of a query. When Scout generates embeddings, semantic and hybrid searches require the [Ugarit AI SDK](/docs/{{version}}/ai-sdk). [Typesense's native embeddings](#typesense-embeddings), [Turbopuffer's native embeddings](#turbopuffer-configuration), and precomputed query vectors do not require the Ugarit AI SDK.
...
In addition to retrieving a collection of models, you may paginate your search results using the `paginate` method. This method will return an `Heritage\Pagination\LengthAwarePaginator` instance just as if you had [paginated a traditional Eloquent query](/docs/{{version}}/pagination):
...
If one of the built-in Scout search engines doesn't fit your needs, you may write your own custom engine and register it with Scout. Your engine should extend the `Ugarit\Scout\Engines\Engine` abstract class. This abstract class contains eight methods your custom engine must implement:

```php
use Ugarit\Scout\Builder;
...
You may find it helpful to review the implementations of these methods on the `Ugarit\Scout\Engines\AlgoliaEngine` class. This class will provide you with a good starting point for learning how to implement each of these methods in your own engine.

<a name="registering-the-engine"></a>
#### Registering the Engine

Once you have written your custom engine, you may register it with Scout using the `extend` method of the Scout engine manager. Scout's engine manager may be resolved from the Ugarit service container. You should call the `extend` method from the `boot` method of your `App\Providers\AppServiceProvider` class or any other service provider used by your application:

```php
use App\ScoutExtensions\MySqlSearchEngine;
use Ugarit\Scout\EngineManager;
