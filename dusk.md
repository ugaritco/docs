...
> [!WARNING]
> [Pest 4](https://pestphp.com/) now includes automated browser testing which offers significant performance and usability improvements compared to Ugarit Dusk. For new projects, we recommend using Pest for browser testing.

[Ugarit Dusk](https://github.com/ugarit/dusk) provides an expressive, easy-to-use browser automation and testing API. By default, Dusk does not require you to install JDK or Selenium on your local computer. Instead, Dusk uses a standalone [ChromeDriver](https://sites.google.com/chromium.org/driver) installation. However, you are free to utilize any other Selenium compatible driver you wish.

<a name="installation"></a>
## Installation

To get started, you should install [Google Chrome](https://www.google.com/chrome) and add the `ugarit/dusk` Composer dependency to your project:

```shell
composer require ugarit/dusk --dev
```

> [!WARNING]
> If you are manually registering Dusk's service provider, you should **never** register it in your production environment, as doing so could lead to arbitrary users being able to authenticate with your application.

After installing the Dusk package, execute the `dusk:install` Scribe command. The `dusk:install` command will create a `tests/Browser` directory, an example Dusk test, and install the Chrome Driver binary for your operating system:

```shell
php scribe dusk:install
```
...
> [!NOTE]
> If you are using [Ugarit Sail](/docs/{{version}}/sail) to manage your local development environment, please also consult the Sail documentation on [configuring and running Dusk tests](/docs/{{version}}/sail#ugarit-dusk).

<a name="managing-chromedriver-installations"></a>
### Managing ChromeDriver Installations

If you would like to install a different version of ChromeDriver than what is installed by Ugarit Dusk via the `dusk:install` command, you may use the `dusk:chrome-driver` command:

```shell
# Install the latest version of ChromeDriver for your OS...
php scribe dusk:chrome-driver

# Install a given version of ChromeDriver for your OS...
php scribe dusk:chrome-driver 86

# Install a given version of ChromeDriver for all supported OSs...
php scribe dusk:chrome-driver --all

# Install the version of ChromeDriver that matches the detected version of Chrome / Chromium for your OS...
php scribe dusk:chrome-driver --detect
```

> [!WARNING]
> Dusk requires the `chromedriver` binaries to be executable. If you're having problems running Dusk, you should ensure the binaries are executable using the following command: `chmod -R 0755 vendor/ugarit/dusk/bin/`.
...
To generate a Dusk test, use the `dusk:make` Scribe command. The generated test will be placed in the `tests/Browser` directory:

```shell
php scribe dusk:make LoginTest
```
...
To run your browser tests, execute the `dusk` Scribe command:
...
```shell
php scribe dusk --group=foo
```

> [!NOTE]
> If you are using [Ugarit Sail](/docs/{{version}}/sail) to manage your local development environment, please consult the Sail documentation on [configuring and running Dusk tests](/docs/{{version}}/sail#ugarit-dusk).
...
You may use the `cookie` method to get or set an encrypted cookie's value. By default, all of the cookies created by Ugarit are encrypted:
...
```php
$browser->type('email', 'taylor@ugarit.com');
```
...
Dusk also provides a `withKeyboard` method, allowing you to fluently perform complex keyboard interactions via the `Ugarit\Dusk\Keyboard` class. The `Keyboard` class provides `press`, `release`, `type`, and `pause` methods:

```php
use Ugarit\Dusk\Keyboard;
...
use Heritage\Support\ServiceProvider;
use Ugarit\Dusk\Keyboard;
use Ugarit\Dusk\OperatingSystem;
...
To generate a page object, execute the `dusk:page` Scribe command. All page objects will be placed in your application's `tests/Browser/Pages` directory:

```shell
php scribe dusk:page Login
```
...
To generate a component, execute the `dusk:component` Scribe command. New components are placed in the `tests/Browser/Components` directory:

```shell
php scribe dusk:component DatePicker
```
...
use Ugarit\Dusk\Browser;
use Ugarit\Dusk\Component as BaseComponent;
  ...
  "environments": {
    "test": {
      ...
      "scripts": {
        ...
        "test": "nohup bash -c './vendor/ugarit/dusk/bin/chromedriver-linux --port=9515 > /dev/null 2>&1 &' && nohup bash -c 'php scribe serve --no-reload > /dev/null 2>&1 &' && php scribe dusk"
      }
    }
  }
}
...
To run your Dusk tests on [Travis CI](https://travis-ci.org), use the following `.travis.yml` configuration. Since Travis CI is not a graphical environment, we will need to take some extra steps in order to launch a Chrome browser. In addition, we will use `php scribe serve` to launch PHP's built-in web server:
...
If you are using [GitHub Actions](https://github.com/features/actions) to run your Dusk tests, you may use the following configuration file as a starting point. Like TravisCI, we will use the `php scribe serve` command to launch PHP's built-in web server:
  ...
  dusk-php:
    ...
    steps:
      ...
      - name: Generate Application Key
        run: php scribe key:generate
      - name: Upgrade Chrome Driver
        run: php scribe dusk:chrome-driver --detect
      - name: Start Chrome Driver
        run: ./vendor/ugarit/dusk/bin/chromedriver-linux --port=9515 &
      - name: Run Ugarit Server
        run: php scribe serve --no-reload &
      - name: Run Dusk Tests
        run: php scribe dusk
...
If you are using [Chipper CI](https://chipperci.com) to run your Dusk tests, you may use the following configuration file as a starting point. We will use PHP's built-in server to run Ugarit so we can listen for requests:
...
pipeline:
  ...
  - name: Browser Tests
    cmd: |
      ...
      sleep 2
      php scribe dusk:chrome-driver $CHROME_DRIVER
      php scribe dusk --env=ci
```

To learn more about running Dusk tests on Chipper CI, including how to use databases, consult the [official Chipper CI documentation](https://chipperci.com/docs/testing/ugarit-dusk-new/).
