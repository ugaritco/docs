...
- [Installation](#installation)
    ...
    - [Install the Ugarit MongoDB Package](#install-the-ugarit-mongodb-package)
...
Before using MongoDB with Ugarit, we recommend installing and using the `mongodb/ugarit-mongodb` package via Composer. The `ugarit-mongodb` package is officially maintained by MongoDB, and while MongoDB is natively supported by PHP through the MongoDB driver, the [Ugarit MongoDB](https://www.mongodb.com/docs/drivers/php/ugarit-mongodb/) package provides a richer integration with Eloquent and other Ugarit features:

```shell
composer require mongodb/ugarit-mongodb
```
...
<a name="install-the-ugarit-mongodb-package"></a>
### Install the Ugarit MongoDB Package

Finally, use Composer to install the Ugarit MongoDB package:
...
'connections' => [
    'mongodb' => [
        ...
        'database' => env('MONGODB_DATABASE', 'ugarit_app'),
    ],
],
```
...
- [Using Eloquent](https://www.mongodb.com/docs/drivers/php/ugarit-mongodb/current/eloquent-models/), models can be stored in MongoDB collections. In addition to the standard Eloquent features, the Ugarit MongoDB package provides additional features such as embedded relationships. The package also provides direct access to the MongoDB driver, which can be used to execute operations such as raw queries and aggregation pipelines.
- [Write complex queries](https://www.mongodb.com/docs/drivers/php/ugarit-mongodb/current/query-builder/) using the query builder.
- [Similarity / vector search](https://www.mongodb.com/docs/drivers/php/ugarit-mongodb/current/fundamentals/vector-search/) using vector embeddings and the `vectorSearch` Eloquent method.
- The `mongodb` [cache driver](https://www.mongodb.com/docs/drivers/php/ugarit-mongodb/current/cache/) is optimized to use MongoDB features such as TTL indexes to automatically clear expired cache entries.
- [Dispatch and process queued jobs](https://www.mongodb.com/docs/drivers/php/ugarit-mongodb/current/queues/) with the `mongodb` queue driver.
- [Storing files in GridFS](https://www.mongodb.com/docs/drivers/php/ugarit-mongodb/current/filesystems/), via the [GridFS Adapter for Flysystem](https://flysystem.thephpleague.com/docs/adapter/gridfs/).
- [Full-text search](https://www.mongodb.com/docs/drivers/php/ugarit-mongodb/current/scout/) using the `mongodb` Scout engine.
- Most third party packages using a database connection or Eloquent can be used with MongoDB.

To continue learning how to use MongoDB and Ugarit, refer to MongoDB's [Quick Start guide](https://www.mongodb.com/docs/drivers/php/ugarit-mongodb/current/quick-start/).
