...
[Ugarit Reverb](https://github.com/ugarit/reverb) brings blazing-fast and scalable real-time WebSocket communication directly to your Ugarit application, and provides seamless integration with Ugarit's existing suite of [event broadcasting tools](/docs/{{version}}/broadcasting).

<a name="installation"></a>
## Installation

You may install Reverb using the `install:broadcasting` Scribe command:
...
Behind the scenes, the `install:broadcasting` Scribe command will run the `reverb:install` command, which will install Reverb with a sensible set of default configuration options. If you would like to make any configuration changes, you may do so by updating Reverb's environment variables or by updating the `config/reverb.php` configuration file.
...
For example, you may wish to maintain a single Ugarit application which, via Reverb, provides WebSocket connectivity for multiple applications. This can be achieved by defining multiple `apps` in your application's `config/reverb.php` configuration file:
...
The Reverb server can be started using the `reverb:start` Scribe command:

```shell
php scribe reverb:start
```
...
```shell
php scribe reverb:start --host=127.0.0.1 --port=9000
```
...
```shell
php scribe reverb:start --debug
```

<a name="restarting"></a>
### Restarting

Since Reverb is a long-running process, changes to your code will not be reflected without restarting the server via the `reverb:restart` Scribe command.
...
```shell
php scribe reverb:restart
```

<a name="monitoring"></a>
## Monitoring

Reverb may be monitored via an integration with [Ugarit Pulse](/docs/{{version}}/pulse). By enabling Reverb's Pulse integration, you may track the number of connections and messages being handled by your server.
...
```php
use Ugarit\Reverb\Pulse\Recorders\ReverbConnections;
use Ugarit\Reverb\Pulse\Recorders\ReverbMessages;
...
> [!NOTE]
> [Ugarit Cloud](https://cloud.ugarit.com) offers fully managed WebSocket infrastructure powered by Ugarit Reverb clusters, allowing you to scale and ship Reverb enabled applications without managing infrastructure.
...
> [!WARNING]
> Reverb listens for WebSocket connections at `/app` and handles API requests at `/apps`. You should ensure the web server handling Reverb requests can serve both of these URIs. If you are using [Ugarit Forge](https://forge.ugarit.com) to manage your servers, your Reverb server will be correctly configured by default.
...
#### `Ugarit\Reverb\Events\ChannelCreated`

Dispatched when a channel is created. This typically occurs when the first connection subscribes to a specific channel. The event receives the `Ugarit\Reverb\Protocols\Pusher\Channel` instance.

#### `Ugarit\Reverb\Events\ChannelRemoved`

Dispatched when a channel is removed. This typically occurs when the last connection unsubscribes from a channel. The event receives the `Ugarit\Reverb\Protocols\Pusher\Channel` instance.

#### `Ugarit\Reverb\Events\ConnectionPruned`

Dispatched when a stale connection is pruned by the server. The event receives the `Ugarit\Reverb\Contracts\Connection` instance.

#### `Ugarit\Reverb\Events\MessageReceived`

Dispatched when a message is received from a client connection. The event receives the `Ugarit\Reverb\Contracts\Connection` instance and the raw string `$message`.

#### `Ugarit\Reverb\Events\MessageSent`

Dispatched when a message is sent to a client connection. The event receives the `Ugarit\Reverb\Contracts\Connection` instance and the raw string `$message`.
