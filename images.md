...
Ugarit provides a fluent image manipulation API that allows you to resize, crop, encode, and store images using the same expressive conventions found throughout the framework. Ugarit's image features are powered by [Intervention Image](https://image.intervention.io/) and support the GD and Imagick PHP extensions.

The image API is useful when working with uploaded files, files stored on Ugarit [filesystem disks](/docs/{{version}}/filesystem), local files, remote URLs, or raw image bytes:

```php
use Heritage\Support\Facades\Image;
...
Before using Ugarit's image manipulation features, install the Intervention Image package via Composer:
...
Ugarit's image configuration file is located at `config/images.php`. If your application does not have an `images` configuration file, you may publish it using the `config:publish` Scribe command:

```shell
php scribe config:publish images
```
...
You may retrieve an uploaded image from an incoming request using the `image` method. This method returns an `Heritage\Image\Image` instance for the uploaded file, or `null` if the file is not present:
...
Alternatively, you may create an image instance from an `Heritage\Http\UploadedFile` instance using the `fromUpload` method:
...
Ugarit also provides a variety of additional image transformation methods:
...
Image instances support Ugarit's `Conditionable` trait, allowing you to conditionally apply transformations using the `when` and `unless` methods:
...
The `store` method stores the processed image on one of your application's filesystem disks. Like uploaded files, Ugarit will generate a unique filename and return the stored path. The second argument may be used to specify the disk:
...
Ugarit's image manager extends Ugarit's base `Heritage\Support\Manager` class. This means you may register custom image drivers using the `extend` method available on the image manager and `Image` facade.

Custom image drivers should implement the `Heritage\Contracts\Image\Driver` interface. The `process` method receives the original image contents and the ordered `Heritage\Image\ImagePipeline` that should be applied to the image, and should return the processed image bytes:
...
use Heritage\Contracts\Image\Driver;
...
> [!NOTE]
> To better understand how to implement a custom image driver, you may review the framework's built-in `Heritage\Image\Drivers\InterventionDriver` class.
...
Applications and packages may define custom transformations by creating a class that implements the `Heritage\Contracts\Image\Transformation` contract. Custom transformations can then be added to an image pipeline using the `transform` method:
