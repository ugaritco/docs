...
All routes and controllers should return a response to be sent back to the user's browser. Ugarit provides several different ways to return responses. The most basic response is returning a string from a route or controller. The framework will automatically convert the string into a full HTTP response:
...
Typically, you won't just be returning simple strings or arrays from your route actions. Instead, you will be returning full `Heritage\Http\Response` instances or [views](/docs/{{version}}/views).
...
You may also return [Eloquent ORM](/docs/{{version}}/eloquent) models and collections directly from your routes and controllers. When you do, Ugarit will automatically convert the models and collections to JSON responses while respecting the model's [hidden attributes](/docs/{{version}}/eloquent-serialization#hiding-attributes-from-json):
...
Ugarit includes a `cache.headers` middleware, which may be used to quickly set the `Cache-Control` header for a group of routes. Directives should be provided using the "snake case" equivalent of the corresponding cache-control directive and should be separated by a semicolon. If `etag` is specified in the list of directives, an MD5 hash of the response content will automatically be set as the ETag identifier:
...
You may attach a cookie to an outgoing `Heritage\Http\Response` instance using the `cookie` method. You should pass the name, value, and the number of minutes the cookie should be considered valid to this method:
...
```php
use Heritage\Support\Facades\Cookie;
...
By default, thanks to the `Heritage\Cookie\Middleware\EncryptCookies` middleware, all cookies generated by Ugarit are encrypted and signed so that they can't be modified or read by the client. If you would like to disable encryption for a subset of cookies generated by your application, you may use the `encryptCookies` method in your application's `bootstrap/app.php` file:
...
Redirect responses are instances of the `Heritage\Http\RedirectResponse` class, and contain the proper headers needed to redirect the user to another URL. There are several ways to generate a `RedirectResponse` instance. The simplest method is to use the global `redirect` helper:
...
When you call the `redirect` helper with no parameters, an instance of `Heritage\Routing\Redirector` is returned, allowing you to call any method on the `Redirector` instance. For example, to generate a `RedirectResponse` to a named route, you may use the `route` method:
...
The `response` helper may be used to generate other types of response instances. When the `response` helper is called without arguments, an implementation of the `Heritage\Contracts\Routing\ResponseFactory` [contract](/docs/{{version}}/contracts) is returned. This contract provides several helpful methods for generating responses.
...
For convenience, if the closure you provide to the `stream` method returns a [Generator](https://www.php.net/manual/en/language.generators.overview.php), Ugarit will automatically flush the output buffer between strings returned by the generator, as well as disable Nginx output buffering:
...
Streamed responses may be consumed using Ugarit's `stream` npm package, which provides a convenient API for interacting with Ugarit response and event streams. To get started, install the `@ugarit/stream-react`, `@ugarit/stream-vue`, or `@ugarit/stream-svelte` package:

```shell tab=React
npm install @ugarit/stream-react
```

```shell tab=Vue
npm install @ugarit/stream-vue
```

```shell tab=Svelte
npm install @ugarit/stream-svelte
```

Then, `useStream` may be used to consume the event stream. After providing your stream URL, the hook will automatically update the `data` with the concatenated response as content is returned from your Ugarit application:

```tsx tab=React
import { useStream } from "@ugarit/stream-react";
...
import { useStream } from "@ugarit/stream-vue";
...
```svelte tab=Svelte
<script>
import { useStream } from "@ugarit/stream-svelte";
...
```tsx tab=React
import { useJsonStream } from "@ugarit/stream-react";
...
```vue tab=Vue
<script setup lang="ts">
import { useJsonStream } from "@ugarit/stream-vue";
...
```svelte tab=Svelte
<script>
import { useJsonStream } from "@ugarit/stream-svelte";
...
Event streams may be consumed using Ugarit's `stream` npm package, which provides a convenient API for interacting with Ugarit event streams. To get started, install the `@ugarit/stream-react`, `@ugarit/stream-vue`, or `@ugarit/stream-svelte` package:
...
Then, `useEventStream` may be used to consume the event stream. After providing your stream URL, the hook will automatically update the `message` with the concatenated response as messages are returned from your Ugarit application:

```jsx tab=React
import { useEventStream } from "@ugarit/stream-react";
...
```vue tab=Vue
<script setup lang="ts">
import { useEventStream } from "@ugarit/stream-vue";
...
```svelte tab=Svelte
<script>
import { useEventStream } from "@ugarit/stream-svelte";
