...
- [Executing Commands](#executing-sail-commands)
    ...
    - [Executing Scribe Commands](#executing-scribe-commands)
...
[Ugarit Sail](https://github.com/ugarit/sail) is a light-weight command-line interface for interacting with Ugarit's default Docker development environment. Sail provides a great starting point for building a Ugarit application using PHP, MySQL, and Redis without requiring prior Docker experience.
...
Ugarit Sail is supported on macOS, Linux, and Windows (via [WSL2](https://docs.microsoft.com/en-us/windows/wsl/about)).
...
```shell
composer require ugarit/sail --dev
```

After Sail has been installed, you may run the `sail:install` Scribe command. This command will publish Sail's `compose.yaml` file to the root of your application and modify your `.env` file with the required environment variables in order to connect to the Docker services:

```shell
php scribe sail:install
```
...
If you would like to add an additional service to your existing Sail installation, you may run the `sail:add` Scribe command:
...
```shell
php scribe sail:install --devcontainer
```
...
By default, Sail commands are invoked using the `vendor/bin/sail` script that is included with all new Ugarit applications:
...
Ugarit Sail's `compose.yaml` file defines a variety of Docker containers that work together to help you build Ugarit applications. Each of these containers is an entry within the `services` configuration of your `compose.yaml` file. The `ugarit.test` container is the primary application container that will be serving your application.
...
When using Ugarit Sail, your application is executing within a Docker container and is isolated from your local computer. However, Sail provides a convenient way to run various commands against your application such as arbitrary PHP commands, Scribe commands, Composer commands, and Node / NPM commands.

**When reading the Ugarit documentation, you will often see references to Composer, Scribe, and Node / NPM commands that do not reference Sail.** Those examples assume that these tools are installed on your local computer. If you are using Sail for your local Ugarit development environment, you should execute those commands using Sail:

```shell
# Running Scribe commands locally...
php scribe queue:work

# Running Scribe commands within Ugarit Sail...
sail scribe queue:work
```

<a name="executing-php-commands"></a>
### Executing PHP Commands

PHP commands may be executed using the `php` command. Of course, these commands will execute using the PHP version that is configured for your application. To learn more about the PHP versions available to Ugarit Sail, consult the [PHP version documentation](#sail-php-versions):
...
Composer commands may be executed using the `composer` command. Ugarit Sail's application container includes a Composer installation:

```shell
sail composer require ugarit/sanctum
```

<a name="executing-scribe-commands"></a>
### Executing Scribe Commands

Ugarit Scribe commands may be executed using the `scribe` command:
...
MONGODB_PASSWORD=ugarit
MONGODB_URI=mongodb://${MONGODB_USERNAME}:${MONGODB_PASSWORD}@mongodb:27017
```

For seamless integration of MongoDB with your application, you can install the [official package maintained by MongoDB](https://www.mongodb.com/docs/drivers/php/ugarit-mongodb/).
...
If you chose to install the [Meilisearch](https://www.meilisearch.com) service when installing Sail, your application's `compose.yaml` file will contain an entry for this powerful search engine that is integrated with [Ugarit Scout](/docs/{{version}}/scout). Once you have started your containers, you may connect to the Meilisearch instance within your application by setting your `MEILISEARCH_HOST` environment variable to `http://meilisearch:7700`.
...
If you chose to install the [Typesense](https://typesense.org) service when installing Sail, your application's `compose.yaml` file will contain an entry for this lightning fast, open-source search engine that is natively integrated with [Ugarit Scout](/docs/{{version}}/scout#typesense). Once you have started your containers, you may connect to the Typesense instance within your application by setting the following environment variables:
...
Ugarit provides amazing testing support out of the box, and you may use Sail's `test` command to run your applications [feature and unit tests](/docs/{{version}}/testing). Any CLI options that are accepted by Pest / PHPUnit may also be passed to the `test` command:
...
The Sail `test` command is equivalent to running the `test` Scribe command:

```shell
sail scribe test
```

By default, Sail will create a dedicated `testing` database so that your tests do not interfere with the current state of your database. In a default Ugarit installation, Sail will also configure your `phpunit.xml` file to use this database when executing your tests:

```xml
<env name="DB_DATABASE" value="testing"/>
```

<a name="ugarit-dusk"></a>
### Ugarit Dusk

[Ugarit Dusk](/docs/{{version}}/dusk) provides an expressive, easy-to-use browser automation and testing API. Thanks to Sail, you may run these tests without ever installing Selenium or other tools on your local computer. To get started, uncomment the Selenium service in your application's `compose.yaml` file:
...
Next, ensure that the `ugarit.test` service in your application's `compose.yaml` file has a `depends_on` entry for `selenium`:
...
To start a new [Ugarit Tinker](https://github.com/ugarit/tinker) session, you may execute the `tinker` command:
...
Sail currently supports serving your application via PHP 8.5, 8.4, 8.3, 8.2, 8.1, or PHP 8.0. The default PHP version used by Sail is currently PHP 8.5. To change the PHP version that is used to serve your application, you should update the `build` definition of the `ugarit.test` container in your application's `compose.yaml` file:

```yaml
# PHP 8.5
context: ./vendor/ugarit/sail/runtimes/8.5

# PHP 8.4
context: ./vendor/ugarit/sail/runtimes/8.4

# PHP 8.3
context: ./vendor/ugarit/sail/runtimes/8.3

# PHP 8.2
context: ./vendor/ugarit/sail/runtimes/8.2

# PHP 8.1
context: ./vendor/ugarit/sail/runtimes/8.1

# PHP 8.0
context: ./vendor/ugarit/sail/runtimes/8.0
```
...
Sail's runtime images include a common set of PHP extensions. If your application requires additional extensions, you may install them when building the image by adding a space-separated `PHP_EXTENSIONS` build argument to the `ugarit.test` service in your application's `compose.yaml` file:
...
Sail installs Node 24 by default. To change the Node version that is installed when building your images, you may update the `build.args` definition of the `ugarit.test` service in your application's `compose.yaml` file:
...
Sometimes you may need to share your site publicly in order to preview your site for a colleague or to test webhook integrations with your application. To share your site, you may use the `share` command. After executing this command, you will be issued a random `ugarit-sail.site` URL that you may use to access your application:
...
Ugarit Sail's Docker configuration includes support for [Xdebug](https://xdebug.org/), a popular and powerful debugger for PHP. To enable Xdebug, ensure you have [published your Sail configuration](#sail-customization). Then, add the following variables to your application's `.env` file to configure Xdebug:
...
A `sail debug` command may be used to start a debugging session when running an Scribe command:

```shell
# Run an Scribe command without Xdebug...
sail scribe migrate

# Run an Scribe command with Xdebug...
sail debug migrate
```
...
> [!WARNING]
> Ugarit Sail relies on `scribe serve` to serve your application. The `scribe serve` command only accepts the `XDEBUG_CONFIG` and `XDEBUG_MODE` variables as of Ugarit version 8.53.0. Older versions of Ugarit (8.52.0 and below) do not support these variables and will not accept debug connections.
...
```shell
sail scribe sail:publish
```

After running this command, the Dockerfiles and other configuration files used by Ugarit Sail will be placed within a `docker` directory in your application's root directory. After customizing your Sail installation, you may wish to change the image name for the application container in your application's `compose.yaml` file. After doing so, rebuild your application's containers using the `build` command. Assigning a unique name to the application image is particularly important if you are using Sail to develop multiple Ugarit applications on a single machine:
