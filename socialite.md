...
In addition to typical, form based authentication, Ugarit also provides a simple, convenient way to authenticate with OAuth providers using [Ugarit Socialite](https://github.com/ugarit/socialite). Socialite currently supports authentication via Facebook, X, LinkedIn, Google, GitHub, GitLab, Bitbucket, and Slack.
...
```shell
composer require ugarit/socialite
```

<a name="upgrading-socialite"></a>
## Upgrading Socialite

When upgrading to a new major version of Socialite, it's important that you carefully review [the upgrade guide](https://github.com/ugarit/socialite/blob/master/UPGRADE.md).
...
```php
use Ugarit\Socialite\Socialite;
...
Ugarit Socialite provides a convenient way to test OAuth authentication flows without making actual requests to OAuth providers. The `fake` method allows you to mock the OAuth provider's behavior and define the user data that should be returned.
...
OAuth 1 users may be faked using the `Ugarit\Socialite\One\User` class.
