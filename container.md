...
The Ugarit service container is a powerful tool for managing class dependencies and performing dependency injection. Dependency injection is a fancy phrase that essentially means this: class dependencies are "injected" into the class via the constructor or, in some cases, "setter" methods.
...
A deep understanding of the Ugarit service container is essential to building a powerful, large application, as well as for contributing to the Ugarit core itself.
...
Thankfully, many of the classes you will be writing when building a Ugarit application automatically receive their dependencies via the container, including [controllers](/docs/{{version}}/controllers), [event listeners](/docs/{{version}}/events), [middleware](/docs/{{version}}/middleware), and more. Additionally, you may type-hint dependencies in the `handle` method of [queued jobs](/docs/{{version}}/queues). Once you taste the power of automatic and zero configuration dependency injection it feels impossible to develop without it.

<a name="when-to-use-the-container"></a>
### When to Utilize the Container

Thanks to zero configuration resolution, you will often type-hint dependencies on routes, controllers, event listeners, and elsewhere without ever manually interacting with the container. For example, you might type-hint the `Heritage\Http\Request` object on your route definition so that you can easily access the current request. Even though we never have to interact with the container to write this code, it is managing the injection of these dependencies behind the scenes:
...
In many cases, thanks to automatic dependency injection and [facades](/docs/{{version}}/facades), you can build Ugarit applications without **ever** manually binding or resolving anything from the container. **So, when would you ever manually interact with the container?** Let's examine two situations.

First, if you write a class that implements an interface and you wish to type-hint that interface on a route or class constructor, you must [tell the container how to resolve that interface](#binding-interfaces-to-implementations). Secondly, if you are [writing a Ugarit package](/docs/{{version}}/packages) that you plan to share with other Ugarit developers, you may need to bind your package's services into the container.
...
For convenience, you may omit providing the class or interface name that you wish to register as a separate argument and instead allow Ugarit to infer the type from the return type of the closure you provide to the `bind` method:
...
use Heritage\Container\Attributes\Singleton;
...
The `scoped` method binds a class or interface into the container that should only be resolved one time within a given Ugarit request / job lifecycle. While this method is similar to the `singleton` method, instances registered using the `scoped` method will be flushed whenever the Ugarit application starts a new "lifecycle", such as when a [Ugarit Octane](/docs/{{version}}/octane) worker processes a new request or when a Ugarit [queue worker](/docs/{{version}}/queues) processes a new job:
...
Alternatively, you may mark an interface or class with the `#[Scoped]` attribute to indicate to the container that it should be resolved one time within a given Ugarit request / job lifecycle:
...
use Heritage\Container\Attributes\Scoped;
...
Ugarit also provides a `Bind` attribute for added convenience. You can apply this attribute to any interface to tell Ugarit which implementation should be automatically injected whenever that interface is requested. When using the `Bind` attribute, there is no need to perform any additional service registration in your application's service providers.
...
use App\Services\RedisEventPusher;
use Heritage\Container\Attributes\Bind;
...
use Heritage\Container\Attributes\BindWhen;
...
Sometimes you may have two classes that utilize the same interface, but you wish to inject different implementations into each class. For example, two controllers may depend on different implementations of the `Heritage\Contracts\Filesystem\Filesystem` [contract](/docs/{{version}}/contracts). Ugarit provides a simple, fluent interface for defining this behavior:
...
Since contextual binding is often used to inject implementations of drivers or configuration values, Ugarit offers a variety of contextual binding attributes that allow to inject these types of values without manually defining the contextual bindings in your service providers.
...
use Heritage\Container\Attributes\Storage;
use Heritage\Contracts\Filesystem\Filesystem;
...
In addition to the `Storage` attribute, Ugarit offers `Auth`, `Cache`, `Config`, `Context`, `DB`, `Give`, `Log`, `RequestAttribute`, `RouteParameter`, and [Tag](#tagging) attributes:
...
use Heritage\Container\Attributes\Auth;
use Heritage\Container\Attributes\Cache;
use Heritage\Container\Attributes\Config;
use Heritage\Container\Attributes\Context;
use Heritage\Container\Attributes\DB;
use Heritage\Container\Attributes\Give;
use Heritage\Container\Attributes\Log;
use Heritage\Container\Attributes\RequestAttribute;
use Heritage\Container\Attributes\RouteParameter;
use Heritage\Container\Attributes\Tag;
use Heritage\Contracts\Auth\Guard;
...
In addition, Ugarit provides a `CurrentUser` attribute for injecting the currently authenticated user into a given route or class:

```php
use App\Models\User;
use Heritage\Container\Attributes\CurrentUser;
...
You can create your own contextual attributes by implementing the `Heritage\Contracts\Container\ContextualAttribute` contract. The container will call your attribute's `resolve` method, which should resolve the value that should be injected into the class utilizing the attribute. In the example below, we will re-implement Ugarit's built-in `Config` attribute:
...
If you would like to have the Ugarit container instance itself injected into a class that is being resolved by the container, you may type-hint the `Heritage\Container\Container` class on your class's constructor:
...
Ugarit's service container implements the [PSR-11](https://github.com/php-fig/fig-standards/blob/master/accepted/PSR-11-container.md) interface. Therefore, you may type-hint the PSR-11 container interface to obtain an instance of the Ugarit container:
