...
Ugarit provides several helpers to assist you in generating URLs for your application. These helpers are primarily helpful when building links in your templates and API responses, or when generating redirect responses to another part of your application.
...
```php
echo url()->query('/posts', ['search' => 'Ugarit']);

// https://example.com/posts?search=Ugarit

echo url()->query('/posts?sort=latest', ['search' => 'Ugarit']);

// http://example.com/posts?sort=latest&search=Ugarit
```
...
If no path is provided to the `url` helper, an `Heritage\Routing\UrlGenerator` instance is returned, allowing you to access information about the current URL:
...
Ugarit allows you to easily create "signed" URLs to named routes. These URLs have a "signature" hash appended to the query string which allows Ugarit to verify that the URL has not been modified since it was created. Signed URLs are especially useful for routes that are publicly accessible yet need a layer of protection against URL manipulation.
...
If you would like to generate a temporary signed route URL that expires after a specified amount of time, you may use the `temporarySignedRoute` method. When Ugarit validates a temporary signed route URL, it will ensure that the expiration timestamp that is encoded into the signed URL has not elapsed:
...
To verify that an incoming request has a valid signature, you should call the `hasValidSignature` method on the incoming `Heritage\Http\Request` instance:
...
```php
use Heritage\Routing\Exceptions\InvalidSignatureException;
...
Ugarit's `Uri` class provides a convenient and fluent interface for creating and manipulating URIs via objects. This class wraps the functionality provided by the underlying League URI package and integrates seamlessly with Ugarit's routing system.
