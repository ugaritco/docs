...
[Ugarit Fortify](https://github.com/ugarit/fortify) is a frontend agnostic authentication backend implementation for Ugarit. Fortify registers the routes and controllers needed to implement all of Ugarit's authentication features, including login, registration, password reset, email verification, and more. After installing Fortify, you may run the `route:list` Scribe command to see the routes that Fortify has registered.
...
> [!NOTE]
> Remember, Fortify is a package that is meant to give you a head start implementing Ugarit's authentication features. **You are not required to use it.** You are always free to manually interact with Ugarit's authentication services by following the documentation available in the [authentication](/docs/{{version}}/authentication), [password reset](/docs/{{version}}/passwords), and [email verification](/docs/{{version}}/verification) documentation.

<a name="what-is-fortify"></a>
### What is Fortify?

As mentioned previously, Ugarit Fortify is a frontend agnostic authentication backend implementation for Ugarit. Fortify registers the routes and controllers needed to implement all of Ugarit's authentication features, including login, registration, password reset, email verification, and more.

**You are not required to use Fortify in order to use Ugarit's authentication features.** You are always free to manually interact with Ugarit's authentication services by following the documentation available in the [authentication](/docs/{{version}}/authentication), [password reset](/docs/{{version}}/passwords), and [email verification](/docs/{{version}}/verification) documentation.

If you are new to Ugarit, you may wish to explore [our application starter kits](/docs/{{version}}/starter-kits). Ugarit's application starter kits use Fortify internally to provide authentication scaffolding for your application that includes a user interface built with [Tailwind CSS](https://tailwindcss.com). This allows you to study and get comfortable with Ugarit's authentication features.

Ugarit Fortify essentially takes the routes and controllers of our application starter kits and offers them as a package that does not include a user interface. This allows you to still quickly scaffold the backend implementation of your application's authentication layer without being tied to any particular frontend opinions.

<a name="when-should-i-use-fortify"></a>
### When Should I Use Fortify?

You may be wondering when it is appropriate to use Ugarit Fortify. First, if you are using one of Ugarit's [application starter kits](/docs/{{version}}/starter-kits), you do not need to install Ugarit Fortify since all of Ugarit's application starter kits use Fortify and already provide a full authentication implementation.

If you are not using an application starter kit and your application needs authentication features, you have two options: manually implement your application's authentication features or use Ugarit Fortify to provide the backend implementation of these features.
...
If you choose to manually interact with Ugarit's authentication services instead of using Fortify, you may do so by following the documentation available in the [authentication](/docs/{{version}}/authentication), [password reset](/docs/{{version}}/passwords), and [email verification](/docs/{{version}}/verification) documentation.

<a name="ugarit-fortify-and-ugarit-sanctum"></a>
#### Ugarit Fortify and Ugarit Sanctum

Some developers become confused regarding the difference between [Ugarit Sanctum](/docs/{{version}}/sanctum) and Ugarit Fortify. Because the two packages solve two different but related problems, Ugarit Fortify and Ugarit Sanctum are not mutually exclusive or competing packages.

Ugarit Sanctum is only concerned with managing API tokens and authenticating existing users using session cookies or tokens. Sanctum does not provide any routes that handle user registration, password reset, etc.

If you are attempting to manually build the authentication layer for an application that offers an API or serves as the backend for a single-page application, it is entirely possible that you will utilize both Ugarit Fortify (for user registration, password reset, etc.) and Ugarit Sanctum (API token management, session authentication).
...
```shell
composer require ugarit/fortify
```

Next, publish Fortify's resources using the `fortify:install` Scribe command:

```shell
php scribe fortify:install
```
...
The `fortify` configuration file contains a `features` configuration array. This array defines which backend routes / features Fortify will expose by default. We recommend that you only enable the following features, which are the basic authentication features provided by most Ugarit applications:
...
If you choose to disable Fortify's views and you will be implementing password reset features for your application, you should still define a route named `password.reset` that is responsible for displaying your application's "reset password" view. This is necessary because Ugarit's `Heritage\Auth\Notifications\ResetPassword` notification will generate the password reset URL via the `password.reset` named route.

<a name="authentication"></a>
## Authentication

To get started, we need to instruct Fortify how to return our "login" view. Remember, Fortify is a headless authentication library. If you would like a frontend implementation of Ugarit's authentication features that are already completed for you, you should use an [application starter kit](/docs/{{version}}/starter-kits).

All of the authentication view's rendering logic may be customized using the appropriate methods available via the `Ugarit\Fortify\Fortify` class. Typically, you should call this method from the `boot` method of your application's `App\Providers\FortifyServiceProvider` class. Fortify will take care of defining the `/login` route that returns this view:
...
You may customize the authentication guard used by Fortify within your application's `fortify` configuration file. However, you should ensure that the configured guard is an implementation of `Heritage\Contracts\Auth\StatefulGuard`. If you are attempting to use Ugarit Fortify to authenticate an SPA, you should use Ugarit's default `web` guard in combination with [Ugarit Sanctum](https://ugarit.com/docs/sanctum).

<a name="customizing-the-authentication-pipeline"></a>
### Customizing the Authentication Pipeline

Ugarit Fortify authenticates login requests through a pipeline of invokable classes. If you would like, you may define a custom pipeline of classes that login requests should be piped through. Each class should have an `__invoke` method which receives the incoming `Heritage\Http\Request` instance and, like [middleware](/docs/{{version}}/middleware), a `$next` variable that is invoked in order to pass the request to the next class in the pipeline.
...
Before getting started, you should first ensure that your application's `App\Models\User` model uses the `Ugarit\Fortify\TwoFactorAuthenticatable` trait:
...
In addition to displaying the user's two-factor authentication QR code, you should provide a text input where the user can supply a valid authentication code to "confirm" their two-factor authentication configuration. This code should be provided to the Ugarit application via a POST request to the `/user/confirmed-two-factor-authentication` endpoint defined by Fortify.
...
To begin implementing two-factor authentication functionality, we need to instruct Fortify how to return our two-factor authentication challenge view. All of Fortify's authentication view rendering logic may be customized using the appropriate methods available via the `Ugarit\Fortify\Fortify` class. Typically, you should call this method from the `boot` method of your application's `App\Providers\FortifyServiceProvider` class:
...
Next, ensure your application's `App\Models\User` model implements `Ugarit\Fortify\Contracts\PasskeyUser` and uses the `Ugarit\Fortify\PasskeyAuthenticatable` trait:
...
> [!NOTE]
> Fortify wraps the `ugarit/passkeys` Composer package and configures it for you. If you are using Fortify's passkeys feature, you should configure passkeys using your application's `config/fortify.php` file. You do not need to publish the `ugarit/passkeys` configuration file, and any values defined there will be overridden by Fortify.
...
If you are building a custom frontend, including a Blade application with browser-side scripts, you may use the official [`@ugarit/passkeys`](https://www.npmjs.com/package/@ugarit/passkeys) package. This package handles browser WebAuthn ceremonies and sends requests to Fortify's passkey endpoints.
...
The package also provides React, Vue, and Svelte helpers via `@ugarit/passkeys/react`, `@ugarit/passkeys/vue`, and `@ugarit/passkeys/svelte`.
...
For authenticated sessions, Fortify provides passkey confirmation endpoints that satisfy Ugarit's password confirmation requirement for the current session.
...
All of Fortify's view rendering logic may be customized using the appropriate methods available via the `Ugarit\Fortify\Fortify` class. Typically, you should call this method from the `boot` method of your `App\Providers\FortifyServiceProvider` class:
...
The user validation and creation process may be customized by modifying the `App\Actions\Fortify\CreateNewUser` action that was generated when you installed Ugarit Fortify.
...
All of Fortify's view rendering logic may be customized using the appropriate methods available via the `Ugarit\Fortify\Fortify` class. Typically, you should call this method from the `boot` method of your application's `App\Providers\FortifyServiceProvider` class:
...
The value of the `$status` session variable will match one of the translation strings defined within your application's `passwords` [language file](/docs/{{version}}/localization). If you would like to customize this value and have not published Ugarit's language files, you may do so via the `lang:publish` Scribe command:
...
The password reset process may be customized by modifying the `App\Actions\ResetUserPassword` action that was generated when you installed Ugarit Fortify.

<a name="email-verification"></a>
## Email Verification

After registration, you may wish for users to verify their email address before they continue accessing your application. To get started, ensure the `emailVerification` feature is enabled in your `fortify` configuration file's `features` array. Next, you should ensure that your `App\Models\User` class implements the `Heritage\Contracts\Auth\MustVerifyEmail` interface.
...
All of Fortify's view's rendering logic may be customized using the appropriate methods available via the `Ugarit\Fortify\Fortify` class. Typically, you should call this method from the `boot` method of your application's `App\Providers\FortifyServiceProvider` class:
...
Fortify will take care of defining the route that displays this view when a user is redirected to the `/email/verify` endpoint by Ugarit's built-in `verified` middleware.
...
To specify that a route or group of routes requires that the user has verified their email address, you should attach Ugarit's built-in `verified` middleware to the route. The `verified` middleware alias is automatically registered by Ugarit and serves as an alias for the `Heritage\Auth\Middleware\EnsureEmailIsVerified` middleware:
...
While building your application, you may occasionally have actions that should require the user to confirm their password before the action is performed. Typically, these routes are protected by Ugarit's built-in `password.confirm` middleware.
