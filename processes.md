...
Ugarit provides an expressive, minimal API around the [Symfony Process component](https://symfony.com/doc/current/components/process.html), allowing you to conveniently invoke external processes from your Ugarit application. Ugarit's process features are focused on the most common use cases and a wonderful developer experience.
...
Of course, the `Heritage\Contracts\Process\ProcessResult` instance returned by the `run` method offers a variety of helpful methods that may be used to inspect the process result:
...
If you have a process result and would like to throw an instance of `Heritage\Process\Exceptions\ProcessFailedException` if the exit code is greater than zero (thus indicating failure), you may use the `throw` and `throwIf` methods. If the process did not fail, the `ProcessResult` instance will be returned:
...
Of course, you may need to customize the behavior of a process before invoking it. Thankfully, Ugarit allows you to tweak a variety of process features, such as the working directory, timeout, and environment variables.
...
By default, processes will throw an instance of `Heritage\Process\Exceptions\ProcessTimedOutException` after executing for more than 60 seconds. However, you can customize this behavior via the `timeout` method:
...
Ugarit also offers the `seeInOutput` and `seeInErrorOutput` methods, which provide a convenient way to determine if a given string was contained in the process' output:

```php
if (Process::run('ls -la')->seeInOutput('ugarit')) {
    // ...
}
```
...
```php
use Heritage\Process\Pipe;
use Heritage\Support\Facades\Process;

$result = Process::pipe(function (Pipe $pipe) {
    $pipe->command('cat example.txt');
    $pipe->command('grep -i "ugarit"');
});
...
Ugarit also allows you to assign string keys to each process within a pipeline via the `as` method. This key will also be passed to the output closure provided to the `pipe` method, allowing you to determine which process the output belongs to:

```php
$result = Process::pipe(function (Pipe $pipe) {
    $pipe->as('first')->command('cat example.txt');
    $pipe->as('second')->command('grep -i "ugarit"');
...
Instead of waiting until the process has finished, you may use the `waitUntil` method to stop waiting based on the output of the process. Ugarit will stop waiting for the process to finish when the closure given to the `waitUntil` method returns `true`:
...
Ugarit also makes it a breeze to manage a pool of concurrent, asynchronous processes, allowing you to easily execute many tasks simultaneously. To get started, invoke the `pool` method, which accepts a closure that receives an instance of `Heritage\Process\Pool`.
...
Accessing process pool results via a numeric key is not very expressive; therefore, Ugarit allows you to assign string keys to each process within a pool via the `as` method. This key will also be passed to the closure provided to the `start` method, allowing you to determine which process the output belongs to:
...
Many Ugarit services provide functionality to help you easily and expressively write tests, and Ugarit's process service is no exception. The `Process` facade's `fake` method allows you to instruct Ugarit to return stubbed / dummy results when processes are invoked.

<a name="faking-processes"></a>
### Faking Processes

To explore Ugarit's ability to fake processes, let's imagine a route that invokes a process:
...
When testing this route, we can instruct Ugarit to return a fake, successful process result for every invoked process by calling the `fake` method on the `Process` facade with no arguments. In addition, we can even [assert](#available-assertions) that a given process was "run":

```php tab=Pest
<?php

use Heritage\Contracts\Process\ProcessResult;
use Heritage\Process\PendingProcess;
use Heritage\Support\Facades\Process;
...
As discussed, invoking the `fake` method on the `Process` facade will instruct Ugarit to always return a successful process result with no output. However, you may easily specify the output and exit code for faked processes using the `Process` facade's `result` method:
...
As [previously discussed](#faking-processes), Ugarit provides several process assertions for your feature tests. We'll discuss each of these assertions below.
...
```php
Process::assertRan(['php', 'scribe', 'migrate']);
```
...
The `$process` passed to the `assertRan` closure is an instance of `Heritage\Process\PendingProcess`, while the `$result` is an instance of `Heritage\Contracts\Process\ProcessResult`.
