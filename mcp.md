...
[Ugarit MCP](https://github.com/ugarit/mcp) provides a simple and elegant way for AI clients to interact with your Ugarit application through the [Model Context Protocol](https://modelcontextprotocol.io/docs/getting-started/intro). It offers an expressive, fluent interface for defining servers, tools, resources, and prompts that enable AI-powered interactions with your application.

<a name="installation"></a>
## Installation

To get started, install Ugarit MCP into your project using the Composer package manager:

```shell
composer require ugarit/mcp
```

<a name="publishing-routes"></a>
### Publishing Routes

After installing Ugarit MCP, execute the `vendor:publish` Scribe command to publish the `routes/ai.php` file where you will define your MCP servers:

```shell
php scribe vendor:publish --tag=ai-routes
```
...
You can create an MCP server using the `make:mcp-server` Scribe command. Servers act as the central communication point that exposes MCP capabilities like tools, resources, and prompts to AI clients:

```shell
php scribe make:mcp-server WeatherServer
```

This command will create a new server class in the `app/Mcp/Servers` directory. The generated server class extends Ugarit MCP's base `Ugarit\Mcp\Server` class and provides attributes and properties for configuring the server and registering tools, resources, and prompts:
...
use Ugarit\Mcp\Server\Attributes\Instructions;
use Ugarit\Mcp\Server\Attributes\Name;
use Ugarit\Mcp\Server\Attributes\Version;
...
class WeatherServer extends Server
{
     ...
     *
     * @var array<int, class-string<\Ugarit\Mcp\Server\Tool>>
     */
     ...
     *
     * @var array<int, class-string<\Ugarit\Mcp\Server\Resource>>
     */
     ...
     *
     * @var array<int, class-string<\Ugarit\Mcp\Server\Prompt>>
     */
...
Once you've created a server, you must register it in your `routes/ai.php` file to make it accessible. Ugarit MCP provides two methods for registering servers: `web` for HTTP-accessible servers and `local` for command-line servers.
...
Local servers run as Scribe commands, perfect for building local AI assistant integrations like [Ugarit Boost](/docs/{{version}}/installation#installing-ugarit-boost). Register a local server using the `local` method:
...
Once registered, you should not typically need to manually run the `mcp:start` Scribe command yourself. Instead, configure your MCP client (AI agent) to start the server or use the [MCP Inspector](#mcp-inspector).

<a name="cache-hints"></a>
### Cache Hints

Ugarit MCP includes cache hints with responses that may be cached, such as server discovery, primitive listings, and resource reads. By default, these responses are marked as private with a time to live of zero milliseconds.

You may customize the default cache hint for a server using the `Cacheable` attribute:

```php
use Ugarit\Mcp\Enums\CacheScope;
use Ugarit\Mcp\Server\Attributes\Cacheable;
...
class WeatherServer extends Server
{
     ...
     *
     * @return array<string, \Ugarit\Mcp\Server\Attributes\Cacheable>
     */
...
use Ugarit\Mcp\Server\Attributes\Description;
...
class CurrentWeatherTool extends Tool
{
     ...
     *
     * @return array<string, \Heritage\JsonSchema\Types\Type>
     */
...
To create a tool, run the `make:mcp-tool` Scribe command:

```shell
php scribe make:mcp-tool CurrentWeatherTool
```
...
use Ugarit\Mcp\Server;
use Ugarit\Mcp\Server\Tools\ToolSearch;
...
class WeatherServer extends Server
{
     ...
     * The tools registered with this MCP server.
     *
     * @var array<int|string, \Ugarit\Mcp\Server\Tool|class-string<\Ugarit\Mcp\Server\Tool>|array<int, \Ugarit\Mcp\Server\Tool|class-string<\Ugarit\Mcp\Server\Tool>>>
     */
    protected array $tools = [
        CurrentWeatherTool::class,
...
```php
use Ugarit\Mcp\Server\Attributes\Name;
use Ugarit\Mcp\Server\Attributes\Title;
...
Tools can define input schemas to specify what arguments they accept from AI clients. Use Ugarit's `Heritage\Contracts\JsonSchema\JsonSchema` builder to define your tool's input requirements:
...
Ugarit MCP integrates seamlessly with Ugarit's [validation features](/docs/{{version}}/validation). You may validate incoming tool arguments within your tool's `handle` method:
...
The Ugarit [service container](/docs/{{version}}/container) is used to resolve all tools. As a result, you are able to type-hint any dependencies your tool may need in its constructor. The declared dependencies will automatically be resolved and injected into the tool instance:
...
use Ugarit\Mcp\Server\Tools\Annotations\IsIdempotent;
...
```php
use Ugarit\Mcp\Server\Tools\Annotations\IsReadOnly;
use Ugarit\Mcp\Server\Tools\Annotations\IsDestructive;
use Ugarit\Mcp\Server\Tools\Annotations\IsOpenWorld;
use Ugarit\Mcp\Server\Tools\Annotations\IsIdempotent;
use Ugarit\Mcp\Server\Tool;
...
Tools must return an instance of `Ugarit\Mcp\Response`. The Response class provides several convenient methods for creating different types of responses:
...
You may also load image and audio content directly from a Ugarit filesystem disk using the `fromStorage` method. The MIME type will be automatically detected from the file:
 ...
 * Handle the tool request.
 *
 * @return array<int, \Ugarit\Mcp\Response>
 */
...
class CurrentWeatherTool extends Tool
{
     ...
     * Handle the tool request.
     *
     * @return \Generator<int, \Ugarit\Mcp\Response>
     */
...
To create a prompt, run the `make:mcp-prompt` Scribe command:

```shell
php scribe make:mcp-prompt DescribeWeatherPrompt
```
...
use Ugarit\Mcp\Server\Prompts\Argument;
...
class DescribeWeatherPrompt extends Prompt
{
     ...
     *
     * @return array<int, \Ugarit\Mcp\Server\Prompts\Argument>
     */
...
Ugarit MCP integrates seamlessly with Ugarit's [validation features](/docs/{{version}}/validation). You may validate incoming prompt arguments within your prompt's `handle` method:
...
The Ugarit [service container](/docs/{{version}}/container) is used to resolve all prompts. As a result, you are able to type-hint any dependencies your prompt may need in its constructor. The declared dependencies will automatically be resolved and injected into the prompt instance:
...
Prompts may return a single `Ugarit\Mcp\Response` or an iterable of `Ugarit\Mcp\Response` instances. These responses encapsulate the content that will be sent to the AI client:
...
To create a resource, run the `make:mcp-resource` Scribe command:

```shell
php scribe make:mcp-resource WeatherGuidelinesResource
```
...
use Ugarit\Mcp\Response;
use Ugarit\Mcp\Server\Attributes\Description;
use Ugarit\Mcp\Server\Attributes\MimeType;
use Ugarit\Mcp\Server\Contracts\HasUriTemplate;
use Ugarit\Mcp\Server\Resource;
use Ugarit\Mcp\Support\UriTemplate;
...
use Ugarit\Mcp\Server\Attributes\MimeType;
use Ugarit\Mcp\Server\Attributes\Uri;
use Ugarit\Mcp\Server\Resource;
...
The Ugarit [service container](/docs/{{version}}/container) is used to resolve all resources. As a result, you are able to type-hint any dependencies your resource may need in its constructor. The declared dependencies will automatically be resolved and injected into the resource instance:
...
use Ugarit\Mcp\Enums\Role;
use Ugarit\Mcp\Server\Annotations\Audience;
use Ugarit\Mcp\Server\Annotations\LastModified;
use Ugarit\Mcp\Server\Annotations\Priority;
use Ugarit\Mcp\Server\Resource;
...
Resources must return an instance of `Ugarit\Mcp\Response`. The Response class provides several convenient methods for creating different types of responses:
...
You may create an app resource using the `make:mcp-app-resource` Scribe command:

```shell
php scribe make:mcp-app-resource WeatherDashboardApp
```
...
use Ugarit\Mcp\Request;
use Ugarit\Mcp\Response;
use Ugarit\Mcp\Server\Attributes\AppMeta;
use Ugarit\Mcp\Server\Attributes\Description;
use Ugarit\Mcp\Server\AppResource;
...
To display an app resource, link a tool to it using the `#[RendersApp]` attribute. When the tool is called, Ugarit MCP includes the resource's URI in the tool metadata so the host can render the app in a sandboxed iframe:
...
use Ugarit\Mcp\Request;
use Ugarit\Mcp\Response;
use Ugarit\Mcp\Server\Attributes\RendersApp;
use Ugarit\Mcp\Server\Tool;
...
Ugarit MCP automatically advertises the `io.modelcontextprotocol/ui` extension within the server's `extensions` capability whenever any `AppResource` is registered, so no additional server configuration is required.
...
```php
use Ugarit\Mcp\Server\Attributes\RendersApp;
use Ugarit\Mcp\Server\Ui\Enums\Visibility;
...
```php
use Ugarit\Mcp\Server\Attributes\AppMeta;
use Ugarit\Mcp\Server\Ui\Enums\Library;
use Ugarit\Mcp\Server\Ui\Enums\Permission;
...
For computed or dynamic configuration, override the `appMeta` method on your resource using the fluent `AppMeta`, `Csp`, and `Permissions` builders from the `Ugarit\Mcp\Server\Ui` namespace.

<a name="building-apps-with-boost"></a>
### Building Apps With Boost

Ugarit MCP includes a dedicated [Boost](/docs/{{version}}/boost) skill reference for building MCP Apps. If you have [Ugarit Boost](/docs/{{version}}/boost) installed, your AI coding agent can invoke the `mcp-development` skill and ask it to scaffold an app resource, Blade view, and linked tool for you.
...
use Ugarit\Mcp\Request;
use Ugarit\Mcp\Response;
use Ugarit\Mcp\ResponseFactory;
...
```php
use Ugarit\Mcp\Enums\IconTheme;
use Ugarit\Mcp\Server\Attributes\Icon;
...
- Relative paths are resolved to a URL using Ugarit's `asset` helper.
...
There are two ways to authenticate access to your MCP server: simple, token based authentication via [Ugarit Sanctum](/docs/{{version}}/sanctum) or any token which is passed via the `Authorization` HTTP header. Or, you may authenticate via OAuth using [Ugarit Passport](/docs/{{version}}/passport).

<a name="oauth"></a>
### OAuth 2.1

The most robust way to protect your web-based MCP servers is with OAuth using [Ugarit Passport](/docs/{{version}}/passport).
...
If your application is not already using Ugarit Passport, follow Passport's [installation and deployment guide](/docs/{{version}}/passport#installation) to add Passport to your application. You should have an `OAuthenticatable` model, new authentication guard, and passport keys before moving on.

Next, you should publish Ugarit MCP's provided Passport authorization view:

```shell
php scribe vendor:publish --tag=mcp-views
```
...
If your application is already using Ugarit Passport, Ugarit MCP should work seamlessly within your existing Passport installation, but custom scopes aren't currently supported as OAuth is primarily used as a translation layer to the underlying authenticatable model.

Ugarit MCP, via the `Mcp::oauthRoutes` method discussed above, adds, advertises, and uses a single `mcp:use` scope.
...
In addition to building servers, Ugarit MCP includes a client for connecting to other MCP servers, whether first-party or third-party. The client lets your application discover and call the tools exposed by an MCP server, which is especially useful for giving your [AI agents](/docs/{{version}}/ai-sdk#mcp-tools) access to capabilities provided by external MCP servers.
...
$client = Client::local('php', ['scribe', 'mcp:start']);
```
...
> [!NOTE]
> The `clientId` and `clientSecret` arguments may be omitted. Ugarit will use a [Client ID Metadata Document](https://modelcontextprotocol.io/specification/2026-07-28/basic/authorization/client-registration#client-id-metadata-documents) when the authorization server supports them, falling back to [dynamic client registration](https://datatracker.ietf.org/doc/html/rfc7591) for legacy servers.

The authorization server must advertise support for the `S256` PKCE code challenge method in its authorization server metadata. Ugarit will reject the authorization attempt if PKCE support is not advertised.
...
```php
use Heritage\Support\Facades\Auth;
use Ugarit\Mcp\Client\OAuth\TokenSet;
use Ugarit\Mcp\Facades\Mcp;
...
```shell
# Web server...
php scribe mcp:inspector mcp/weather

# Local server named "weather"...
php scribe mcp:inspector weather
```
