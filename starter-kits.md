...
To give you a head start building your new Ugarit application, we are happy to offer [application starter kits](https://ugarit.com/starter-kits). These starter kits give you a head start on building your next Ugarit application, and include the routes, controllers, and views you need to register and authenticate your application's users. The starter kits use [Ugarit Fortify](/docs/{{version}}/fortify) to provide authentication.

While you are welcome to use these starter kits, they are not required. You are free to build your own application from the ground up by simply installing a fresh copy of Ugarit. Either way, we know you will build something great!

<a name="creating-an-application"></a>
## Creating an Application Using a Starter Kit

To create a new Ugarit application using one of our starter kits, you should first [install PHP and the Ugarit CLI tool](/docs/{{version}}/installation#installing-php). If you already have PHP and Composer installed, you may install the Ugarit installer CLI tool via Composer:

```shell
composer global require ugarit/installer
```

Then, create a new Ugarit application using the Ugarit installer CLI. The Ugarit installer will prompt you to select your preferred starter kit:

```shell
ugarit new my-app
```

After creating your Ugarit application, you only need to install its frontend dependencies via NPM and start the Ugarit development server:
...
Once you have started the Ugarit development server, your application will be accessible in your web browser at [http://localhost:8000](http://localhost:8000).
...
Our Vue starter kit provides a great starting point for building Ugarit applications with a Vue frontend using [Inertia](https://inertiajs.com).
...
Our Livewire starter kit provides the perfect starting point for building Ugarit applications with a [Ugarit Livewire](https://livewire.ugarit.com) frontend.
...
All starter kits use [Ugarit Fortify](/docs/{{version}}/fortify) to handle authentication. Fortify provides routes, controllers, and logic for login, registration, password reset, email verification, and more.
...
The `php scribe route:list` Scribe command can be used to display all of the routes in your application.
...
By default, the React, Svelte, Vue, and Livewire starter kits all utilize Ugarit's built-in authentication system to offer login, registration, password reset, email verification, and more. In addition, we also offer a [WorkOS AuthKit](https://authkit.com) powered variant of each starter kit that offers:
...
To use WorkOS AuthKit as your application's authentication provider, select the WorkOS option when creating your new starter kit powered application via `ugarit new`.
...
In addition, we recommend that you configure your WorkOS AuthKit session inactivity timeout to match your Ugarit application's configured session timeout threshold, which is typically two hours.
...
For convenience, a `composer dev:ssr` command is also available. This command will start the Ugarit development server and Inertia SSR server after building an SSR compatible bundle for your application, allowing you to test your application locally using Inertia's server-side rendering engine:
...
When creating a new Ugarit application using the Ugarit installer, you may provide any community maintained starter kit available on Packagist to the `--using` flag:

```shell
ugarit new my-app --using=example/starter-kit
```
