...
The [Ugarit AI SDK](https://github.com/ugarit/ai) provides a unified, expressive API for interacting with AI providers such as OpenAI, Anthropic, Gemini, and more. With the AI SDK, you can build intelligent agents with tools and structured output, generate images, synthesize and transcribe audio, create vector embeddings, and much more — all using a consistent, Ugarit-friendly interface.

<a name="installation"></a>
## Installation

You can install the Ugarit AI SDK via Composer:

```shell
composer require ugarit/ai
```

Next, you should publish the AI SDK configuration and migration files using the `vendor:publish` Scribe command:

```shell
php scribe vendor:publish --provider="Ugarit\Ai\AiServiceProvider"
```
...
By default, the Ugarit AI SDK connects directly to each provider's public API endpoint. However, you may need to route requests through a different endpoint - for example, when using a proxy service to centralize API key management, implement rate limiting, or route traffic through a corporate gateway.
...
```php
agent()->prompt('What is Ugarit?', provider: 'local', model: 'local-model');
```
...
The `Ugarit\Ai\Enums\Lab` enum may be used to reference providers throughout your code instead of using plain strings:

```php
use Ugarit\Ai\Enums\Lab;
...
Agents are the fundamental building block for interacting with AI providers in the Ugarit AI SDK. Each agent is a dedicated PHP class that encapsulates the instructions, conversation context, tools, and output schema needed to interact with a large language model. Think of an agent as a specialized assistant — a sales coach, a document analyzer, a support bot — that you configure once and prompt as needed throughout your application.

You can create an agent via the `make:agent` Scribe command:

```shell
php scribe make:agent SalesCoach

php scribe make:agent SalesCoach --structured
```
...
use Ugarit\Ai\Contracts\Agent;
use Ugarit\Ai\Contracts\Conversational;
use Ugarit\Ai\Contracts\HasStructuredOutput;
use Ugarit\Ai\Contracts\HasTools;
use Ugarit\Ai\Messages\Message;
use Ugarit\Ai\Promptable;
use Stringable;
...
$response->raw; // Heritage\Http\Client\Response|null
...
> **Warning:** Before using the `RemembersConversations` trait, you should publish and run the AI SDK migrations using the `vendor:publish` Scribe command. These migrations will create the necessary database tables to store conversations.

If you would like Ugarit to automatically store and retrieve conversation history for your agent, you may use the `RemembersConversations` trait. This trait provides a simple way to persist conversation messages to the database without manually implementing the `Conversational` interface:
...
use Ugarit\Ai\Concerns\RemembersConversations;
use Ugarit\Ai\Contracts\Agent;
use Ugarit\Ai\Contracts\Conversational;
use Ugarit\Ai\Promptable;
...
use Ugarit\Ai\Concerns\HasConversations;
...
$response = (new SalesCoach)->prompt(
    ...
    attachments: [
        ...
        Files\Document::fromPath('/home/ugarit/transcript.md'), // Attach a document from a local path...
        $request->file('transcript'), // Attach an uploaded file...
    ]
);
```

Likewise, the `Ugarit\Ai\Files\Image` class may be used to attach images to a prompt:
...
$response = (new ImageAnalyzer)->prompt(
    ...
    attachments: [
        ...
        Files\Image::fromPath('/home/ugarit/photo.jpg'), // Attach an image from a local path...
...
```php
use App\Ai\Agents\SalesCoach;
use Ugarit\Ai\Responses\StreamedAgentResponse;
...
use Ugarit\Ai\Attributes\WithoutBroadcasting;
use Ugarit\Ai\Contracts\Agent;
use Ugarit\Ai\Contracts\HasTools;
use Ugarit\Ai\Promptable;
use Ugarit\Ai\Streaming\Events\ToolCall;
use Ugarit\Ai\Streaming\Events\ToolResult;
...
use Ugarit\Ai\Responses\AgentResponse;
use Throwable;
...
Tools may be used to give agents additional functionality that they can utilize while responding to prompts. Tools can be created using the `make:tool` Scribe command:

```shell
php scribe make:tool RandomNumberGenerator
```
...
use Heritage\Contracts\JsonSchema\JsonSchema;
use Ugarit\Ai\Contracts\Tool;
use Ugarit\Ai\Tools\Request;
use Stringable;
...
Use the `RepairToolCalls` attribute to let an agent recover when a model calls an unknown local tool. Ugarit returns the failed call to the model with the names of the available local tools, allowing it to correct the call:

```php
use Ugarit\Ai\Attributes\RepairToolCalls;
use Ugarit\Ai\Contracts\Agent;
use Ugarit\Ai\Contracts\HasTools;
use Ugarit\Ai\Promptable;
...
When Ugarit derives the maximum number of steps automatically, this attribute adds one step for the repaired call. Explicit `MaxSteps` limits are unchanged.
...
use Ugarit\Ai\Tools\SimilaritySearch;
...
use Ugarit\Ai\Providers\Tools\ToolSearch;
...
The `FileStorage` tool factory allows you to give agents access to a Ugarit [filesystem disk](/docs/{{version}}/filesystem). The `all` method returns tools that allow the agent to list, read, inspect, generate URLs for, write, delete, and copy files on the given disk:

```php
use Ugarit\Ai\Tools\FileStorage;
...
These methods return an `Heritage\Support\Collection`, allowing you to further filter the tools that are provided to the agent:

```php
use Ugarit\Ai\Tools\Filesystem\DeleteFile;
...
If your application uses [Ugarit MCP](/docs/{{version}}/mcp), you may give your agents tools exposed by [Model Context Protocol](https://modelcontextprotocol.io) servers. Using the [Ugarit MCP client](/docs/{{version}}/mcp#client), you may connect to a remote or local MCP server and pass its tools directly to your agent.

> [!NOTE]
> MCP tools require the [Ugarit MCP](/docs/{{version}}/mcp) package to be installed in your application.

Because an MCP client's `tools` method returns a collection, spread it into your agent's `tools` array using the `...` operator:

```php
use App\Ai\Tools\RandomNumberGenerator;
use Ugarit\Mcp\Client;
...
{
    return [
        ...Client::local('php', ['scribe', 'mcp:start'])->tools(),
    ];
}
```
...
```php
use Ugarit\Ai\Providers\Tools\WebSearch;
...
```php
(new WebSearch)->max(5)->allow(['ugarit.com', 'php.net']),
```
...
```php
use Ugarit\Ai\Providers\Tools\WebFetch;
...
```php
(new WebFetch)->max(3)->allow(['docs.ugarit.com']),
```
...
```php
use Ugarit\Ai\Providers\Tools\FileSearch;
...
```php
use Ugarit\Ai\Providers\Tools\FileSearchQuery;
...
use Ugarit\Ai\Attributes\Provider;
use Ugarit\Ai\Contracts\Agent;
use Ugarit\Ai\Contracts\CanActAsTool;
use Ugarit\Ai\Contracts\HasTools;
use Ugarit\Ai\Enums\Lab;
use Ugarit\Ai\Promptable;
...
If a sub-agent does not implement `CanActAsTool`, Ugarit will use the agent's class basename as the tool name and a generic description that asks the parent agent to pass a clear, self-contained task description. Each sub-agent invocation runs in isolation and does not receive the parent agent's conversation history.

<a name="middleware"></a>
### Middleware

Agents support middleware, allowing you to intercept and modify prompts before they are sent to the provider. Middleware can be created using the `make:agent-middleware` Scribe command:

```shell
php scribe make:agent-middleware LogPrompts
```
...
use Ugarit\Ai\Contracts\Agent;
use Ugarit\Ai\Contracts\HasMiddleware;
use Ugarit\Ai\Promptable;
...
use Closure;
use Ugarit\Ai\Prompts\AgentPrompt;
...
```php
use function Ugarit\Ai\{agent};

$response = agent(
    instructions: 'You are an expert at software development.',
    messages: [],
    tools: [],
)->prompt('Tell me about Ugarit')
```
...
use Ugarit\Ai\Attributes\MaxSteps;
use Ugarit\Ai\Attributes\MaxTokens;
use Ugarit\Ai\Attributes\Model;
use Ugarit\Ai\Attributes\Provider;
use Ugarit\Ai\Attributes\Temperature;
use Ugarit\Ai\Attributes\Timeout;
use Ugarit\Ai\Attributes\TopP;
use Ugarit\Ai\Contracts\Agent;
use Ugarit\Ai\Enums\Lab;
use Ugarit\Ai\Promptable;
...
```php
use Ugarit\Ai\Attributes\UseCheapestModel;
use Ugarit\Ai\Attributes\UseSmartestModel;
use Ugarit\Ai\Contracts\Agent;
use Ugarit\Ai\Promptable;
...
> [!NOTE]
> The underlying model selected by `UseCheapestModel` and `UseSmartestModel` may change between releases of the Ugarit AI SDK as providers release new models. Switching models can introduce behavioral changes, deprecated parameters, and significant cost differences. If you need a stable, predictable model and pricing, specify the model explicitly using the `Model` attribute.
...
use Ugarit\Ai\Contracts\Agent;
use Ugarit\Ai\Contracts\HasProviderOptions;
use Ugarit\Ai\Enums\Lab;
use Ugarit\Ai\Promptable;
...
```php
use Ugarit\Ai\Attributes\CacheInstructions;
use Ugarit\Ai\Attributes\CacheToolDefinitions;
...
use Ugarit\Ai\Concerns\InteractsWithApprovals;
use Ugarit\Ai\Contracts\Approvable;
use Ugarit\Ai\Contracts\Tool;
use Ugarit\Ai\Tools\Request;
use Stringable;
...
```php
use Ugarit\Ai\Approvals\Approval;
...
```php
use Ugarit\Ai\Approvals\Decision;
use Ugarit\Ai\Approvals\Decisions;
...
For queued agents, the resulting response is passed to the `then` callback, and Ugarit also dispatches a `ToolApprovalRequested` event.

Ugarit stores the result of an approved tool before asking the model to continue. If generation then fails, the approval has already been resolved. Continue the conversation with a normal text prompt instead of submitting the same approval decisions again.
...
use Ugarit\Ai\Approvals\Decision;
use Ugarit\Ai\Approvals\Decisions;
use Ugarit\Ai\Models\Conversation;
...
The `Ugarit\Ai\Image` class may be used to generate images using the `openai`, `gemini`, or `xai` providers:
...
$image = Image::of('Update this photo of me to be in the style of an impressionist painting.')
    ->attachments([
        ...
        // Files\Image::fromPath('/home/ugarit/photo.jpg'),
...
```php
use Ugarit\Ai\Image;
use Ugarit\Ai\Responses\ImageResponse;
...
The `Ugarit\Ai\Audio` class may be used to generate audio from the given text:

```php
use Ugarit\Ai\Audio;

$audio = Audio::of('I love coding with Ugarit.')->generate();

$rawContent = (string) $audio;
```

You may also generate audio from a string using the `toAudio` method available via Ugarit's `Stringable` class:

```php
use Heritage\Support\Str;

$audio = Str::of('I love coding with Ugarit.')->toAudio();
```

The `male`, `female`, and `voice` methods may be used to determine the voice of the generated audio:

```php
$audio = Audio::of('I love coding with Ugarit.')
    ->female()
    ->generate();
...
```php
use Ugarit\Ai\Audio;
use Ugarit\Ai\Responses\AudioResponse;

Audio::of('I love coding with Ugarit.')
    ->queue()
...
The `Ugarit\Ai\Transcription` class may be used to generate a transcript of the given audio:

```php
use Ugarit\Ai\Transcription;

$transcript = Transcription::fromPath('/home/ugarit/audio.mp3')->generate();
...
```php
use Ugarit\Ai\Transcription;
use Ugarit\Ai\Responses\TranscriptionResponse;
...
You may summarize text using the `summarize` method available via Ugarit's `Stringable` class. By default, the summary will contain no more than three sentences and will be generated using the configured provider's cheapest text model:
...
You may easily generate vector embeddings for any given string using the new `toEmbeddings` method available via Ugarit's `Stringable` class:
...
```php
use Ugarit\Ai\Embeddings;

$response = Embeddings::for([
    'Napa Valley has great wine.',
    'Ugarit is a PHP framework.',
])->generate();
...
use Ugarit\Ai\Embeddings;
use Ugarit\Ai\Enums\Lab;
use Ugarit\Ai\Files\Image;
use Ugarit\Ai\Files\Video;

$response = Embeddings::for([
    'A vineyard at sunset.',
    Image::fromStorage('vineyard.jpg'),
    Video::fromPath('/home/ugarit/tour.mp4'),
])->generate(Lab::Gemini);
```
...
Once you have generated embeddings, you will typically store them in a `vector` column in your database for later querying. Ugarit provides native support for vector columns on PostgreSQL via the `pgvector` extension and MariaDB. To get started, define a `vector` column in your migration, specifying the number of dimensions:
...
You may also add a vector index to speed up similarity searches. When calling `index` on a vector column, Ugarit will automatically create an HNSW index with cosine distance:
...
```php
use Heritage\Database\Eloquent\Casts\AsVector;
...
The `$queryEmbedding` may be an array of floats or a plain string. When a string is given, Ugarit will automatically generate embeddings for it:
...
The `Ugarit\Ai\Reranking` class may be used to rerank documents:

```php
use Ugarit\Ai\Reranking;

$response = Reranking::of([
    'Django is a Python web framework.',
    'Ugarit is a PHP web application framework.',
    'React is a JavaScript library for building user interfaces.',
])->rerank('PHP frameworks');

// Access the top result...
$response->first()->document; // "Ugarit is a PHP web application framework."
...
For convenience, Ugarit collections may be reranked using the `rerank` macro. The first argument specifies which field(s) to use for reranking, and the second argument is the query:

```php
// Rerank by a single field...
$posts = Post::all()
    ->rerank('body', 'Ugarit tutorials');

// Rerank by multiple fields (sent as JSON)...
$reranked = $posts->rerank(['title', 'body'], 'Ugarit tutorials');
...
The `Ugarit\Ai\Files` class or the individual file classes may be used to store files with your AI provider for later use in conversations. This is useful for large documents or files you want to reference multiple times without re-uploading:
...
$response = Document::fromPath('/home/ugarit/document.pdf')->put();
$response = Image::fromPath('/home/ugarit/photo.jpg')->put();
...
$response = Document::fromPath('/home/ugarit/knowledge.txt')
...
$response = Document::fromPath('/home/ugarit/training.jsonl')
...
Vector stores allow you to create searchable collections of files that can be used for retrieval-augmented generation (RAG). The `Ugarit\Ai\Stores` class provides methods for creating, retrieving, and deleting vector stores:

```php
use Ugarit\Ai\Stores;
...
```php
use Ugarit\Ai\Approvals\PendingApproval;
use Ugarit\Ai\Responses\AgentResponse;
...
> **Note:** When `Agent::fake()` is invoked on an agent that returns structured output and fake output was not explicitly provided, Ugarit will automatically generate fake data that matches your agent's defined output schema.
...
```php
use Ugarit\Ai\QueuedAgentPrompt;
...
```php
use Ugarit\Ai\Image;
use Ugarit\Ai\Prompts\ImagePrompt;
use Ugarit\Ai\Prompts\QueuedImagePrompt;
...
```php
use Ugarit\Ai\Audio;
use Ugarit\Ai\Prompts\AudioPrompt;
use Ugarit\Ai\Prompts\QueuedAudioPrompt;
...
```php
use Ugarit\Ai\Transcription;
use Ugarit\Ai\Prompts\TranscriptionPrompt;
use Ugarit\Ai\Prompts\QueuedTranscriptionPrompt;
...
```php
use Ugarit\Ai\Embeddings;
use Ugarit\Ai\Prompts\EmbeddingsPrompt;
use Ugarit\Ai\Prompts\QueuedEmbeddingsPrompt;
...
Embeddings::assertGenerated(function (EmbeddingsPrompt $prompt) {
    return $prompt->contains('Ugarit') && $prompt->dimensions === 1536;
});
...
Embeddings::assertQueued(
    fn (QueuedEmbeddingsPrompt $prompt) => $prompt->contains('Ugarit')
);
...
```php
use Ugarit\Ai\Reranking;
use Ugarit\Ai\Prompts\RerankingPrompt;
use Ugarit\Ai\Responses\Data\RankedDocument;
...
Reranking::assertReranked(function (RerankingPrompt $prompt) {
    return $prompt->contains('Ugarit') && $prompt->limit === 5;
});
...
```php
use Ugarit\Ai\Contracts\Files\StorableFile;
use Ugarit\Ai\Files\Document;

// Store files...
Document::fromString('Hello, Ugarit!', mimeType: 'text/plain')
    ->as('hello.txt')
    ->put();

// Make assertions...
Files::assertStored(fn (StorableFile $file) =>
    (string) $file === 'Hello, Ugarit!' &&
...
The Ugarit AI SDK dispatches a variety of [events](/docs/{{version}}/events), including:
