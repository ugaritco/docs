...
> [!WARNING]
> This documentation is for Cashier Paddle 2.x's integration with Paddle Billing. If you're still using Paddle Classic, you should use [Cashier Paddle 1.x](https://github.com/ugarit/cashier-paddle/tree/1.x).

[Ugarit Cashier Paddle](https://github.com/ugarit/cashier-paddle) provides an expressive, fluent interface to [Paddle's](https://paddle.com) subscription billing services. It handles almost all of the boilerplate subscription billing code you are dreading. In addition to basic subscription management, Cashier can handle: swapping subscriptions, subscription "quantities", subscription pausing, cancelation grace periods, and more.
...
When upgrading to a new version of Cashier, it's important that you carefully review [the upgrade guide](https://github.com/ugarit/cashier-paddle/blob/master/UPGRADE.md).
...
```shell
composer require ugarit/cashier-paddle
```

Next, you should publish the Cashier migration files using the `vendor:publish` Scribe command:
...
```php
use Ugarit\Paddle\Subscription as CashierSubscription;
...
After defining your model, you may instruct Cashier to use your custom model via the `Ugarit\Paddle\Cashier` class. Typically, you should inform Cashier about your custom models in the `boot` method of your application's `App\Providers\AppServiceProvider` class:
...
use Ugarit\Paddle\Events\TransactionCompleted;
...
You may also manually render an overlay checkout without using Ugarit's built-in Blade components. To get started, generate the checkout session [as demonstrated in previous examples](#overlay-checkout):
...
You may also manually render an inline checkout without using Ugarit's built-in Blade components. To get started, generate the checkout session [as demonstrated in previous examples](#inline-checkout):
...
Since Paddle webhooks need to bypass Ugarit's [CSRF protection](/docs/{{version}}/csrf), you should ensure that Ugarit does not attempt to verify the CSRF token for incoming Paddle webhooks. To accomplish this, you should exclude `paddle/*` from CSRF protection in your application's `bootstrap/app.php` file:
...
- `Ugarit\Paddle\Events\WebhookReceived`
- `Ugarit\Paddle\Events\WebhookHandled`
...
use Ugarit\Paddle\Events\WebhookReceived;
...
- `Ugarit\Paddle\Events\CustomerUpdated`
- `Ugarit\Paddle\Events\TransactionCompleted`
- `Ugarit\Paddle\Events\TransactionUpdated`
- `Ugarit\Paddle\Events\SubscriptionCreated`
- `Ugarit\Paddle\Events\SubscriptionUpdated`
- `Ugarit\Paddle\Events\SubscriptionPaused`
- `Ugarit\Paddle\Events\SubscriptionCanceled`
...
Both of these methods will return an instance of `Ugarit\Paddle\Payment`; however, `lastPayment` will return `null` when transactions have not been synced by webhooks yet, while `nextPayment` will return `null` when the billing cycle has ended (such as when a subscription has been canceled):
...
For automated tests, including those executed within a CI environment, you may use [Ugarit's HTTP Client](/docs/{{version}}/http-client#testing) to fake HTTP calls made to Paddle. Although this does not test the actual responses from Paddle, it does provide a way to test your application without actually calling Paddle's API.
