...
[Ugarit Prompts](https://github.com/ugarit/prompts) is a PHP package for adding beautiful and user-friendly forms to your command-line applications, with browser-like features including placeholder text and validation.

<img src="https://ugarit.com/img/docs/prompts-example.png">

Ugarit Prompts is perfect for accepting user input in your [Scribe console commands](/docs/{{version}}/scribe#writing-commands), but it may also be used in any command-line PHP project.

> [!NOTE]
> Ugarit Prompts supports macOS, Linux, and Windows with WSL. For more information, please see our documentation on [unsupported environments & fallbacks](#fallbacks).

<a name="installation"></a>
## Installation

Ugarit Prompts is already included with the latest release of Ugarit.

Ugarit Prompts may also be installed in your other PHP projects by using the Composer package manager:

```shell
composer require ugarit/prompts
```
...
Alternatively, you may leverage the power of Ugarit's [validator](/docs/{{version}}/validation). To do so, provide an array containing the name of the attribute and the desired validation rules to the `validate` argument:
...
```php
use function Ugarit\Prompts\autocomplete;
...
For long running tasks, it can be helpful to show a progress bar that informs users how complete the task is. Using the `progress` function, Ugarit will display a progress bar and advance its progress for each iteration over a given iterable value:
...
The callback may also accept the `Ugarit\Prompts\Progress` instance, allowing you to modify the label and hint on each iteration:
...
task(
    ...
    callback: function ($logger) {
        ...
        // ...
        $logger->line('Downloading ugarit/framework');
        // ...
    }
);
```
...
```php
use function Ugarit\Prompts\stream;
...
Ugarit Prompts supports macOS, Linux, and Windows with WSL. Due to limitations in the Windows version of PHP, it is not currently possible to use Ugarit Prompts on Windows outside of WSL.

For this reason, Ugarit Prompts supports falling back to an alternative implementation such as the [Symfony Console Question Helper](https://symfony.com/doc/current/components/console/helpers/questionhelper.html).

> [!NOTE]
> When using Ugarit Prompts with the Ugarit framework, fallbacks for each prompt have been configured for you and will be automatically enabled in unsupported environments.

<a name="fallback-conditions"></a>
#### Fallback Conditions

If you are not using Ugarit or need to customize when the fallback behavior is used, you may pass a boolean to the `fallbackWhen` static method on the `Prompt` class:
...
If you are not using Ugarit or need to customize the fallback behavior, you may pass a closure to the `fallbackUsing` static method on each prompt class:
...
Ugarit provides a variety of methods for testing that your command displays the expected Prompt messages:

```php tab=Pest
test('report generation', function () {
    $this->scribe('report:generate')
