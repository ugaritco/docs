...
[Ugarit Telescope](https://github.com/ugarit/telescope) makes a wonderful companion to your local Ugarit development environment. Telescope provides insight into the requests coming into your application, exceptions, log entries, database queries, queued jobs, mail, notifications, cache operations, scheduled tasks, variable dumps, and more.

<img src="https://ugarit.com/img/docs/telescope-example.png">

<a name="installation"></a>
## Installation

You may use the Composer package manager to install Telescope into your Ugarit project:

```shell
composer require ugarit/telescope
```

After installing Telescope, publish its assets and migrations using the `telescope:install` Scribe command. After installing Telescope, you should also run the `migrate` command in order to create the tables needed to store Telescope's data:

```shell
php scribe telescope:install
...
```shell
composer require ugarit/telescope --dev
...
{
    if ($this->app->environment('local') && class_exists(\Ugarit\Telescope\TelescopeServiceProvider::class)) {
        $this->app->register(\Ugarit\Telescope\TelescopeServiceProvider::class);
...
use Ugarit\Telescope\Telescope;
...
Without pruning, the `telescope_entries` table can accumulate records very quickly. To mitigate this, you should [schedule](/docs/{{version}}/scheduling) the `telescope:prune` Scribe command to run daily:
...
When upgrading to a new major version of Telescope, it's important that you carefully review [the upgrade guide](https://github.com/ugarit/telescope/blob/master/UPGRADE.md).

In addition, when upgrading to any new Telescope version, you should re-publish Telescope's assets:

```shell
php scribe telescope:publish
```

To keep the assets up-to-date and avoid issues in future updates, you may add the `vendor:publish --tag=ugarit-assets` command to the `post-update-cmd` scripts in your application's `composer.json` file:

```json
{
    "scripts": {
        "post-update-cmd": [
            "@php scribe vendor:publish --tag=ugarit-assets --ansi --force"
        ]
    }
}
```
...
```php
use Ugarit\Telescope\IncomingEntry;
use Ugarit\Telescope\Telescope;
...
```php
use Ugarit\Telescope\EntryType;
use Ugarit\Telescope\IncomingEntry;
use Ugarit\Telescope\Telescope;
...
The command watcher records the arguments, options, exit code, and output whenever an Scribe command is executed. If you would like to exclude certain commands from being recorded by the watcher, you may specify the command in the `ignore` option within your `config/telescope.php` file:
...
The dump watcher records and displays your variable dumps in Telescope. When using Ugarit, variables may be dumped using the global `dump` function. The dump watcher tab must be open in a browser for the dump to be recorded, otherwise, the dumps will be ignored by the watcher.

<a name="event-watcher"></a>
### Event Watcher

The event watcher records the payload, listeners, and broadcast data for any [events](/docs/{{version}}/events) dispatched by your application. The Ugarit framework's internal events are ignored by the Event watcher.
