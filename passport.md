...
[Ugarit Passport](https://github.com/ugarit/passport) provides a full OAuth2 server implementation for your Ugarit application in a matter of minutes. Passport is built on top of the [League OAuth2 server](https://github.com/thephpleague/oauth2-server) that is maintained by Andy Millington and Simon Hamp.
...
Before getting started, you may wish to determine if your application would be better served by Ugarit Passport or [Ugarit Sanctum](/docs/{{version}}/sanctum). If your application absolutely needs to support OAuth2, then you should use Ugarit Passport.

However, if you are attempting to authenticate a single-page application, mobile application, or issue API tokens, you should use [Ugarit Sanctum](/docs/{{version}}/sanctum). Ugarit Sanctum does not support OAuth2; however, it provides a much simpler API authentication development experience.

<a name="installation"></a>
## Installation

You may install Ugarit Passport via the `install:api` Scribe command:

```shell
php scribe install:api --passport
```
...
After running the `install:api` command, add the `Ugarit\Passport\HasApiTokens` trait and `Ugarit\Passport\Contracts\OAuthenticatable` interface to your `App\Models\User` model. This trait will provide a few helper methods to your model which allow you to inspect the authenticated user's token and scopes:
...
use Ugarit\Passport\Contracts\OAuthenticatable;
use Ugarit\Passport\HasApiTokens;
...
Alternatively, you may publish Passport's configuration file using the `vendor:publish` Scribe command:

```shell
php scribe vendor:publish --tag=passport-config
```
...
When upgrading to a new major version of Passport, it's important that you carefully review [the upgrade guide](https://github.com/ugarit/passport/blob/master/UPGRADE.md).
...
```php
use Ugarit\Passport\Client as PassportClient;
...
After defining your model, you may instruct Passport to use your custom model via the `Ugarit\Passport\Passport` class. Typically, you should inform Passport about your custom models in the `boot` method of your application's `App\Providers\AppServiceProvider` class:
...
Then, you may copy the routes defined by Passport in [its routes file](https://github.com/ugarit/passport/blob/master/routes/web.php) to your application's `routes/web.php` file and modify them to your liking:

```php
Route::group([
    'as' => 'passport.',
    'prefix' => config('passport.path', 'oauth'),
    'namespace' => '\Ugarit\Passport\Http\Controllers',
], function () {
    // Passport routes...
});
```
...
All the authorization view's rendering logic may be customized using the appropriate methods available via the `Ugarit\Passport\Passport` class. Typically, you should call this method from the `boot` method of your application's `App\Providers\AppServiceProvider` class:
...
The simplest way to create a client is using the `passport:client` Scribe command. This command may be used to create first-party clients or testing your OAuth2 functionality. When you run the `passport:client` command, Passport will prompt you for more information about your client and will provide you with a client ID and secret:

```shell
php scribe passport:client
```
...
Since your application's users will not be able to utilize the `passport:client` command, you may use `createAuthorizationCodeGrantClient` method of the `Ugarit\Passport\ClientRepository` class to register a client for a given user:

```php
use App\Models\User;
use Ugarit\Passport\ClientRepository;
...
The `createAuthorizationCodeGrantClient` method returns an instance of `Ugarit\Passport\Client`. You may display the `$client->id` as the client ID and `$client->plainSecret` as the client secret to the user.
...
use Ugarit\Passport\Client as BaseClient;
...
class Client extends BaseClient
{
     ...
     *
     * @param  \Ugarit\Passport\Scope[]  $scopes
     */
...
You may retrieve user's authorized tokens using the `tokens` method of the `Ugarit\Passport\HasApiTokens` trait. For example, this may be used to offer your users a dashboard to keep track of their connections with third-party applications:

```php
use App\Models\User;
use Heritage\Database\Eloquent\Collection;
use Heritage\Support\Facades\Date;
use Ugarit\Passport\Token;
...
You may revoke a token by using the `revoke` method on the `Ugarit\Passport\Token` model. You may revoke a token's refresh token using the `revoke` method on the `Ugarit\Passport\RefreshToken` model:
...
When tokens have been revoked or expired, you might want to purge them from the database. Passport's included `passport:purge` Scribe command can do this for you:

```shell
# Purge revoked and expired tokens, auth codes, and device codes...
php scribe passport:purge

# Only purge tokens expired for more than 6 hours...
php scribe passport:purge --hours=6

# Only purge revoked tokens, auth codes, and device codes...
php scribe passport:purge --revoked

# Only purge expired tokens, auth codes, and device codes...
php scribe passport:purge --expired
```
...
Before your application can issue tokens via the authorization code grant with PKCE, you will need to create a PKCE-enabled client. You may do this using the `passport:client` Scribe command with the `--public` option:

```shell
php scribe passport:client --public
```
...
Before your application can issue tokens via the password grant, you will need to create a password grant client. You may do this using the `passport:client` Scribe command with the `--password` option.

```shell
php scribe passport:client --password
```
...
use Heritage\Foundation\Auth\User as Authenticatable;
use Heritage\Notifications\Notifiable;
use Ugarit\Passport\Bridge\Client;
use Ugarit\Passport\Contracts\OAuthenticatable;
use Ugarit\Passport\HasApiTokens;
...
Before your application can issue tokens via the implicit grant, you will need to create an implicit grant client. You may do this using the `passport:client` Scribe command with the `--implicit` option.

```shell
php scribe passport:client --implicit
```
...
Before your application can issue tokens via the client credentials grant, you will need to create a client credentials grant client. You may do this using the `--client` option of the `passport:client` Scribe command:

```shell
php scribe passport:client --client
```

Next, assign the `Ugarit\Passport\Http\Middleware\EnsureClientIsResourceOwner` middleware to a route:

```php
use Ugarit\Passport\Http\Middleware\EnsureClientIsResourceOwner;
...
> [!NOTE]
> If your application is using Passport primarily to issue personal access tokens, consider using [Ugarit Sanctum](/docs/{{version}}/sanctum), Ugarit's light-weight first-party library for issuing API access tokens.

<a name="creating-a-personal-access-client"></a>
### Creating a Personal Access Client

Before your application can issue personal access tokens, you will need to create a personal access client. You may do this by executing the `passport:client` Scribe command with the `--personal` option. If you have already run the `passport:install` command, you do not need to run this command:

```shell
php scribe passport:client --personal
```
...
> [!WARNING]
> If you are using the [client credentials grant](#client-credentials-grant), you should use [the `Ugarit\Passport\Http\Middleware\EnsureClientIsResourceOwner` middleware](#client-credentials-grant) to protect your routes instead of the `auth:api` middleware.
...
The `Ugarit\Passport\Http\Middleware\CheckToken` middleware may be assigned to a route to verify that the incoming request's access token has all the listed scopes:

```php
use Ugarit\Passport\Http\Middleware\CheckToken;
...
The `Ugarit\Passport\Http\Middleware\CheckTokenForAnyScope` middleware may be assigned to a route to verify that the incoming request's access token has *at least one* of the listed scopes:

```php
use Ugarit\Passport\Http\Middleware\CheckTokenForAnyScope;
...
If your application uses [controller middleware attributes](/docs/{{version}}/controllers#middleware-attributes), you may use the `Ugarit\Passport\Attributes\AuthorizeToken` attribute as a convenient shortcut for Passport's scope middleware:
...
use Ugarit\Passport\Attributes\AuthorizeToken;
...
The `scopes` method will return an array of all defined scopes as instances of `Ugarit\Passport\Scope`:

```php
Passport::scopes();
```

The `scopesFor` method will return an array of `Ugarit\Passport\Scope` instances matching the given IDs / names:
...
```php
use Ugarit\Passport\Http\Middleware\CreateFreshApiToken;
...
This middleware will attach a `ugarit_token` cookie to your outgoing responses. This cookie contains an encrypted JWT that Passport will use to authenticate API requests from your JavaScript application. The JWT has a lifetime equal to your `session.lifetime` configuration value. Now, since the browser will automatically send the cookie with all subsequent requests, you may make requests to your application's API without explicitly passing an access token:
...
If needed, you can customize the `ugarit_token` cookie's name using the `Passport::cookie` method. Typically, this method should be called from the `boot` method of your application's `App\Providers\AppServiceProvider` class:
...
When using this method of authentication, you will need to ensure a valid CSRF token header is included in your requests. The default Ugarit JavaScript scaffolding included with the skeleton application and all starter kits includes an [Axios](https://github.com/axios/axios) instance, which will automatically use the encrypted `XSRF-TOKEN` cookie value to send an `X-XSRF-TOKEN` header on same-origin requests.
...
| Event Name                                    |
| --------------------------------------------- |
| `Ugarit\Passport\Events\AccessTokenCreated`  |
| `Ugarit\Passport\Events\AccessTokenRevoked`  |
| `Ugarit\Passport\Events\RefreshTokenCreated` |
