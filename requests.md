...
Ugarit's `Heritage\Http\Request` class provides an object-oriented way to interact with the current HTTP request being handled by your application as well as retrieve the input, cookies, and files that were submitted with the request.
...
To obtain an instance of the current HTTP request via dependency injection, you should type-hint the `Heritage\Http\Request` class on your route closure or controller method. The incoming request instance will automatically be injected by the Ugarit [service container](/docs/{{version}}/container):
...
As mentioned, you may also type-hint the `Heritage\Http\Request` class on a route closure. The service container will automatically inject the incoming request into the closure when it is executed:
...
You may still type-hint the `Heritage\Http\Request` and access your `id` route parameter by defining your controller method as follows:
...
The `Heritage\Http\Request` instance provides a variety of methods for examining the incoming HTTP request and extends the `Symfony\Component\HttpFoundation\Request` class. We will discuss a few of the most important methods below.
...
You may retrieve a request header from the `Heritage\Http\Request` instance using the `header` method. If the header is not present on the request, `null` will be returned. However, the `header` method accepts an optional second argument that will be returned if the header is not present on the request:
...
Ugarit provides several methods for inspecting the incoming request's requested content types via the `Accept` header. First, the `getAcceptableContentTypes` method will return an array containing all of the content types accepted by the request:
...
The [PSR-7 standard](https://www.php-fig.org/psr/psr-7/) specifies interfaces for HTTP messages, including requests and responses. If you would like to obtain an instance of a PSR-7 request instead of a Ugarit request, you will first need to install a few libraries. Ugarit uses the *Symfony HTTP Message Bridge* component to convert typical Ugarit requests and responses into PSR-7 compatible implementations:
...
> [!NOTE]
> If you return a PSR-7 response instance from a route or controller, it will automatically be converted back to a Ugarit response instance and be displayed by the framework.
...
Using a few simple methods, you may access all of the user input from your `Heritage\Http\Request` instance without worrying about which HTTP verb was used for the request. Regardless of the HTTP verb, the `input` method may be used to retrieve user input:
...
Instead of retrieving the request's input data as a primitive `string`, you may use the `string` method to retrieve the request data as an instance of [Heritage\Support\Stringable](/docs/{{version}}/strings):
...
You may also access user input using dynamic properties on the `Heritage\Http\Request` instance. For example, if one of your application's forms contains a `name` field, you may access the value of the field like so:

```php
$name = $request->name;
```

When using dynamic properties, Ugarit will first look for the parameter's value in the request payload. If it is not present, Ugarit will search for the field in the matched route's parameters.
...
Ugarit allows you to keep input from one request during the next request. This feature is particularly useful for re-populating forms after detecting validation errors. However, if you are using Ugarit's included [validation features](/docs/{{version}}/validation), it is possible that you will not need to manually use these session input flashing methods directly, as some of Ugarit's built-in validation facilities will call them automatically.

<a name="flashing-input-to-the-session"></a>
#### Flashing Input to the Session

The `flash` method on the `Heritage\Http\Request` class will flash the current input to the [session](/docs/{{version}}/session) so that it is available during the user's next request to the application:
...
All cookies created by the Ugarit framework are encrypted and signed with an authentication code, meaning they will be considered invalid if they have been changed by the client. To retrieve a cookie value from the request, use the `cookie` method on an `Heritage\Http\Request` instance:
...
You may retrieve uploaded files from an `Heritage\Http\Request` instance using the `file` method or using dynamic properties. The `file` method returns an instance of the `Heritage\Http\UploadedFile` class, which extends the PHP `SplFileInfo` class and provides a variety of methods for interacting with the file:
...
If the uploaded file is an image that you need to manipulate before storing, you may use the `image` method to retrieve an `Heritage\Image\Image` instance, or `null` if the file is not present:
...
> [!NOTE]
> For more information about file storage in Ugarit, check out the complete [file storage documentation](/docs/{{version}}/filesystem).
...
To solve this, you may enable the `Heritage\Http\Middleware\TrustProxies` middleware that is included in your Ugarit application, which allows you to quickly customize the load balancers or proxies that should be trusted by your application. Your trusted proxies should be specified using the `trustProxies` middleware method in your application's `bootstrap/app.php` file:
...
By default, Ugarit will respond to all requests it receives regardless of the content of the HTTP request's `Host` header. In addition, the `Host` header's value will be used when generating absolute URLs to your application during a web request.

Typically, you should configure your web server, such as Nginx or Apache, to only send requests to your application that match a given hostname. However, if you do not have the ability to customize your web server directly and need to instruct Ugarit to only respond to certain hostnames, you may do so by enabling the `Heritage\Http\Middleware\TrustHosts` middleware for your application.
