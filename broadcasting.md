...
To assist you in building these types of features, Ugarit makes it easy to "broadcast" your server-side Ugarit [events](/docs/{{version}}/events) over a WebSocket connection. Broadcasting your Ugarit events allows you to share the same event names and data between your server-side Ugarit application and your client-side JavaScript application.

The core concepts behind broadcasting are simple: clients connect to named channels on the frontend, while your Ugarit application broadcasts events to these channels on the backend. These events can contain any additional data you wish to make available to the frontend.

<a name="supported-drivers"></a>
#### Supported Drivers

By default, Ugarit includes three server-side broadcasting drivers for you to choose from: [Ugarit Reverb](https://reverb.ugarit.com), [Pusher Channels](https://pusher.com/channels), and [Ably](https://ably.com).

> [!NOTE]
> Before diving into event broadcasting, make sure you have read Ugarit's documentation on [events and listeners](/docs/{{version}}/events).

<a name="quickstart"></a>
## Quickstart

By default, broadcasting is not enabled in new Ugarit applications. You may enable broadcasting using the `install:broadcasting` Scribe command:

```shell
php scribe install:broadcasting
```
...
Ugarit supports several broadcast drivers out of the box: [Ugarit Reverb](/docs/{{version}}/reverb), [Pusher Channels](https://pusher.com/channels), [Ably](https://ably.com), and a `log` driver for local development and debugging. Additionally, a `null` driver is included which allows you to disable broadcasting during testing. A configuration example is included for each of these drivers in the `config/broadcasting.php` configuration file.

All of your application's event broadcasting configuration is stored in the `config/broadcasting.php` configuration file. Don't worry if this file does not exist in your application; it will be created when you run the `install:broadcasting` Scribe command.

<a name="quickstart-next-steps"></a>
#### Next Steps

Once you have enabled event broadcasting, you're ready to learn more about [defining broadcast events](#defining-broadcast-events) and [listening for events](#listening-for-events). If you're using Ugarit's React, Vue, or Svelte [starter kits](/docs/{{version}}/starter-kits), you may listen for events using Echo's [useEcho hook](#using-react-or-vue).
...
To get started using Ugarit's event broadcasting, we need to do some configuration within the Ugarit application as well as install a few packages.

Event broadcasting is accomplished by a server-side broadcasting driver that broadcasts your Ugarit events so that Ugarit Echo (a JavaScript library) can receive them within the browser client. Don't worry - we'll walk through each part of the installation process step-by-step.

<a name="reverb"></a>
### Reverb

To quickly enable support for Ugarit's broadcasting features while using Reverb as your event broadcaster, invoke the `install:broadcasting` Scribe command with the `--reverb` option. This Scribe command will install Reverb's required Composer and NPM packages and update your application's `.env` file with the appropriate variables:

```shell
php scribe install:broadcasting --reverb
```

<a name="reverb-manual-installation"></a>
#### Manual Installation

When running the `install:broadcasting` command, you will be prompted to install [Ugarit Reverb](/docs/{{version}}/reverb). Of course, you may also install Reverb manually using the Composer package manager:

```shell
composer require ugarit/reverb
```
...
To quickly enable support for Ugarit's broadcasting features while using Pusher as your event broadcaster, invoke the `install:broadcasting` Scribe command with the `--pusher` option. This Scribe command will prompt you for your Pusher credentials, install the Pusher PHP and JavaScript SDKs, and update your application's `.env` file with the appropriate variables:

```shell
php scribe install:broadcasting --pusher
```
...
Finally, you are ready to install and configure [Ugarit Echo](#client-side-installation), which will receive the broadcast events on the client-side.

<a name="ably"></a>
### Ably

> [!NOTE]
> The documentation below discusses how to use Ably in "Pusher compatibility" mode. However, the Ably team recommends and maintains a broadcaster and Echo client that is able to take advantage of the unique capabilities offered by Ably. For more information on using the Ably maintained drivers, please [consult Ably's Ugarit broadcaster documentation](https://github.com/ably/ugarit-broadcaster).

To quickly enable support for Ugarit's broadcasting features while using [Ably](https://ably.com) as your event broadcaster, invoke the `install:broadcasting` Scribe command with the `--ably` option. This Scribe command will prompt you for your Ably credentials, install the Ably PHP and JavaScript SDKs, and update your application's `.env` file with the appropriate variables:

```shell
php scribe install:broadcasting --ably
```
...
[Ugarit Echo](https://github.com/ugarit/echo) is a JavaScript library that makes it painless to subscribe to channels and listen for events broadcast by your server-side broadcasting driver.

When installing Ugarit Reverb via the `install:broadcasting` Scribe command, Reverb and Echo's scaffolding and configuration will be injected into your application automatically. However, if you wish to manually configure Ugarit Echo, you may do so by following the instructions below.

<a name="reverb-client-manual-installation"></a>
#### Manual Installation

To manually configure Ugarit Echo for your application's frontend, first install the `pusher-js` package since Reverb utilizes the Pusher protocol for WebSocket subscriptions, channels, and messages:

```shell
npm install --save-dev ugarit-echo pusher-js
```

Once Echo is installed, you are ready to create a fresh Echo instance in your application's JavaScript. A great place to do this is at the bottom of the `resources/js/app.js` file that is included with the Ugarit framework:

```js tab=JavaScript
import Echo from 'ugarit-echo';
...
```js tab=React
import { configureEcho } from "@ugarit/echo-react";
...
```js tab=Vue
import { configureEcho } from "@ugarit/echo-vue";
...
```js tab=Svelte
import { configureEcho } from "@ugarit/echo-svelte";
...
> [!WARNING]
> The Ugarit Echo `reverb` broadcaster requires ugarit-echo v1.16.0+.
...
When installing broadcasting support via the `install:broadcasting --pusher` Scribe command, Pusher and Echo's scaffolding and configuration will be injected into your application automatically. However, if you wish to manually configure Ugarit Echo, you may do so by following the instructions below.

<a name="pusher-client-manual-installation"></a>
#### Manual Installation

To manually configure Ugarit Echo for your application's frontend, first install the `ugarit-echo` and `pusher-js` packages which utilize the Pusher protocol for WebSocket subscriptions, channels, and messages:
...
When installing broadcasting support via the `install:broadcasting --ably` Scribe command, Ably and Echo's scaffolding and configuration will be injected into your application automatically. However, if you wish to manually configure Ugarit Echo, you may do so by following the instructions below.
...
Ugarit's event broadcasting allows you to broadcast your server-side Ugarit events to your client-side JavaScript application using a driver-based approach to WebSockets. Currently, Ugarit ships with [Ugarit Reverb](https://reverb.ugarit.com), [Pusher Channels](https://pusher.com/channels), and [Ably](https://ably.com) drivers. The events may be easily consumed on the client-side using the [Ugarit Echo](#client-side-installation) JavaScript package.
...
use Heritage\Broadcasting\Channel;
use Heritage\Broadcasting\InteractsWithSockets;
use Heritage\Broadcasting\PresenceChannel;
...
```php
use Heritage\Broadcasting\Channel;
use Heritage\Broadcasting\PrivateChannel;
 ...
 *
 * @return array<int, \Heritage\Broadcasting\Channel>
 */
...
Next, all that remains is to listen for the event in our JavaScript application. We can do this using [Ugarit Echo](#client-side-installation). Ugarit Echo's built-in React, Vue, and Svelte hooks make it simple to get started, and, by default, all of the event's public properties will be included on the broadcast event:

```js tab=React
import { useEcho } from "@ugarit/echo-react";
...
import { useEcho } from "@ugarit/echo-vue";
...
```svelte tab=Svelte
<script>
import { useEcho } from "@ugarit/echo-svelte";
...
To inform Ugarit that a given event should be broadcast, you must implement the `Heritage\Contracts\Broadcasting\ShouldBroadcast` interface on the event class. This interface is already imported into all event classes generated by the framework so you may easily add it to any of your events.
...
By default, Ugarit will broadcast the event using the event's class name. However, you may customize the broadcast name by defining a `broadcastAs` method on the event:
...
use Heritage\Queue\Attributes\Queue;
...
Private channels require you to authorize that the currently authenticated user can actually listen on the channel. This is accomplished by making an HTTP request to your Ugarit application with the channel name and allowing your application to determine if the user can listen on that channel. When using [Ugarit Echo](#client-side-installation), the HTTP request to authorize subscriptions to private channels will be made automatically.

When broadcasting is installed Ugarit attempts to automatically register the `/broadcasting/auth` route to handle authorization requests. If Ugarit fails to automatically register these routes, you may register them manually in your application's `/bootstrap/app.php` file:
...
Next, we need to define the logic that will actually determine if the currently authenticated user can listen to a given channel. This is done in the `routes/channels.php` file that was created by the `install:broadcasting` Scribe command. In this file, you may use the `Broadcast::channel` method to register channel authorization callbacks:
...
You may view a list of your application's broadcast authorization callbacks using the `channel:list` Scribe command:

```shell
php scribe channel:list
```
...
If your application is consuming many different channels, your `routes/channels.php` file could become bulky. So, instead of using closures to authorize channels, you may use channel classes. To generate a channel class, use the `make:channel` Scribe command. This command will place a new channel class in the `App/Broadcasting` directory.

```shell
php scribe make:channel OrderChannel
```
...
> [!NOTE]
> Like many other classes in Ugarit, channel classes will automatically be resolved by the [service container](/docs/{{version}}/container). So, you may type-hint any dependencies required by your channel in its constructor.
...
> [!WARNING]
> Your event must use the `Heritage\Broadcasting\InteractsWithSockets` trait in order to call the `toOthers` method.

<a name="only-to-others-configuration"></a>
#### Configuration

When you initialize a Ugarit Echo instance, a socket ID is assigned to the connection. If you are using a global [Axios](https://github.com/axios/axios) instance to make HTTP requests from your JavaScript application, the socket ID will automatically be attached to every outgoing request as an `X-Socket-ID` header. Then, when you call the `toOthers` method, Ugarit will extract the socket ID from the header and instruct the broadcaster to not broadcast to any connections with that socket ID.
...
use Heritage\Broadcasting\Channel;
use Heritage\Broadcasting\InteractsWithBroadcasting;
use Heritage\Broadcasting\InteractsWithSockets;
use Heritage\Broadcasting\PresenceChannel;
use Heritage\Broadcasting\PrivateChannel;
use Heritage\Contracts\Broadcasting\ShouldBroadcast;
...
When your application's queue server is unavailable or Ugarit encounters an error while broadcasting an event, an exception is thrown that typically causes the end user to see an application error. Since event broadcasting is often supplementary to your application's core functionality, you can prevent these exceptions from disrupting the user experience by implementing the `ShouldRescue` interface on your events.

Events that implement the `ShouldRescue` interface automatically utilize Ugarit's [rescue helper function](/docs/{{version}}/helpers#method-rescue) during broadcast attempts. This helper catches any exceptions, reports them to your application's exception handler for logging, and allows the application to continue executing normally without interrupting the user's workflow:
...
Once you have [installed and instantiated Ugarit Echo](#client-side-installation), you are ready to start listening for events that are broadcast from your Ugarit application. First, use the `channel` method to retrieve an instance of a channel, then call the `listen` method to listen for a specified event:
...
Ugarit Echo includes React, Vue, and Svelte hooks that make it painless to listen for events. To get started, invoke the `useEcho` hook, which is used to listen for private events. The `useEcho` hook will automatically leave channels when the consuming component is unmounted:
...
```js tab=React
import { useEchoPublic } from "@ugarit/echo-react";
...
```vue tab=Vue
<script setup lang="ts">
import { useEchoPublic } from "@ugarit/echo-vue";
...
```svelte tab=Svelte
<script>
import { useEchoPublic } from "@ugarit/echo-svelte";
...
```js tab=React
import { useEchoPresence } from "@ugarit/echo-react";
...
```vue tab=Vue
<script setup lang="ts">
import { useEchoPresence } from "@ugarit/echo-vue";
...
```svelte tab=Svelte
<script>
import { useEchoPresence } from "@ugarit/echo-svelte";
...
```js tab=React
import { useConnectionStatus } from "@ugarit/echo-react";
...
```vue tab=Vue
<script setup lang="ts">
import { useConnectionStatus } from "@ugarit/echo-vue";
...
```svelte tab=Svelte
<script>
import { useConnectionStatus } from "@ugarit/echo-svelte";
...
```js tab=React
import { useSocketId } from "@ugarit/echo-react";
...
```vue tab=Vue
<script setup lang="ts">
import { useSocketId } from "@ugarit/echo-vue";
...
```svelte tab=Svelte
<script>
import { useSocketId } from "@ugarit/echo-svelte";
...
> [!WARNING]
> Before reading the following documentation about model broadcasting, we recommend you become familiar with the general concepts of Ugarit's model broadcasting services as well as how to manually create and listen to broadcast events.
...
However, if you are not using these events for any other purposes in your application, it can be cumbersome to create event classes for the sole purpose of broadcasting them. To remedy this, Ugarit allows you to indicate that an Eloquent model should automatically broadcast its state changes.

To get started, your Eloquent model should use the `Heritage\Database\Eloquent\BroadcastsEvents` trait. In addition, the model should define a `broadcastOn` method, which will return an array of channels that the model's events should broadcast on:
...
use Heritage\Broadcasting\Channel;
use Heritage\Broadcasting\PrivateChannel;
use Heritage\Database\Eloquent\BroadcastsEvents;
...
class Post extends Model
{
     ...
     *
     * @return array<int, \Heritage\Broadcasting\Channel|\Heritage\Database\Eloquent\Model>
     */
 ...
 * Get the channels that model events should broadcast on.
 *
 * @return array<string, array<int, \Heritage\Broadcasting\Channel|\Heritage\Database\Eloquent\Model>>
 */
public function broadcastOn(string $event): array
{
    return match ($event) {
...
Occasionally, you may wish to customize how Ugarit creates the underlying model broadcasting event. You may accomplish this by defining a `newBroadcastableEvent` method on your Eloquent model. This method should return an `Heritage\Database\Eloquent\BroadcastableModelEventOccurred` instance:

```php
use Heritage\Database\Eloquent\BroadcastableModelEventOccurred;
...
As you may have noticed, the `broadcastOn` method in the model example above did not return `Channel` instances. Instead, Eloquent models were returned directly. If an Eloquent model instance is returned by your model's `broadcastOn` method (or is contained in an array returned by the method), Ugarit will automatically instantiate a private channel instance for the model using the model's class name and primary key identifier as the channel name.

So, an `App\Models\User` model with an `id` of `1` would be converted into an `Heritage\Broadcasting\PrivateChannel` instance with a name of `App.Models.User.1`. Of course, in addition to returning Eloquent model instances from your model's `broadcastOn` method, you may return complete `Channel` instances in order to have full control over the model's channel names:
...
If you plan to explicitly return a channel instance from your model's `broadcastOn` method, you may pass an Eloquent model instance to the channel's constructor. When doing so, Ugarit will use the model channel conventions discussed above to convert the Eloquent model into a channel name string:
...
First, use the `private` method to retrieve an instance of a channel, then call the `listen` method to listen for a specified event. Typically, the channel name given to the `private` method should correspond to Ugarit's [model broadcasting conventions](#model-broadcasting-conventions).
...
If you are using React, Vue, or Svelte, you may use Ugarit Echo's included `useEchoModel` hook to easily listen for model broadcasts:

```js tab=React
import { useEchoModel } from "@ugarit/echo-react";
...
```vue tab=Vue
<script setup lang="ts">
import { useEchoModel } from "@ugarit/echo-vue";
...
```svelte tab=Svelte
<script>
import { useEchoModel } from "@ugarit/echo-svelte";
...
Sometimes you may wish to broadcast an event to other connected clients without hitting your Ugarit application at all. This can be particularly useful for things like "typing" notifications, where you want to alert users of your application that another user is typing a message on a given screen.
